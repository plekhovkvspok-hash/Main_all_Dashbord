...
function fallbackEstimate(input, error) {
  ...
  const amountMatch = String(amountText).replace(',', '.').match(/\d+(\.\d+)?/);
  const amountNumber = amountMatch ? Number(amountMatch[0]) : 1;
  ...
  return {
    ...
    title: workType,
    assumptions: [
      'Смета сформирована в шаблонном режиме: региональные источники нужно проверить вручную.',
    ...
    works,
    totals: {
      materials: materialsTarget,
      labor: laborTarget,
      overhead,
      profit,
      vat,
      reserve: 0,
      total: materialsTarget + laborTarget + overhead + profit + vat,
    },
