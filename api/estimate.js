...
function currentPricingDateLabel() {
  return new Intl.DateTimeFormat('ru-RU', { day: '2-digit', month: 'long', year: 'numeric', timeZone: 'Europe/Moscow' }).format(new Date());
}

function sanitizeCurrentDateText(value) {
  const label = currentPricingDateLabel();
  return String(value || '')
    .replace(/июн[ьяе]?\s+2024\s*года/gi, label)
    .replace(/июн[ьяе]?\s+2024/gi, label)
    .replace(/2024\s*года/gi, '2026 года')
    .replace(/\b2024\b/g, '2026');
}


function amountNumberFromInput(input) {
  const amountMatch = String(input?.amount || '').replace(',', '.').match(/\d+(\.\d+)?/);
  return amountMatch ? Number(amountMatch[0]) : 1;
}

function isAreaUnit(value) {
  return /(кв\.?\s*м|м2|m2|m²|квадрат)/i.test(String(value || ''));
}

function isFinishingWorkText(value) {
  return /(космет|стен|подъезд|штукатур|шпаклев|покраск|окрас|отделоч|трещин|декоратив)/i.test(String(value || ''));
}

function bagQty(value) { return Math.max(1, Math.ceil(value * 10) / 10); }
function literQty(value) { return Math.max(1, Math.ceil(value)); }

function finishingMaterialDefaults(input, sourcePrefix = 'Нормативный пересчет по объему работ') {
  const area = Math.max(1, amountNumberFromInput(input));
  const region = String(input?.region || 'регион');
  const external = input?.workEnvironment === 'external';
  const rows = [
    { name: 'Штукатурная смесь для локального выравнивания', quantity: String(bagQty(area * 0.15)), unit: 'мешок 25 кг', unitPrice: external ? 620 : 520, environment: external ? 'external' : 'internal', sourceNote: sourcePrefix + ': 0,15 мешка на 1 кв.м. для локальных дефектов стен. Для ' + area + ' кв.м. требуется около ' + bagQty(area * 0.15) + ' мешков. Цену сверить по поставщикам региона: ' + region + '.' },
    { name: 'Шпаклевка финишная', quantity: String(bagQty(area * 0.08)), unit: 'мешок 20-25 кг', unitPrice: external ? 780 : 650, environment: external ? 'external' : 'internal', sourceNote: sourcePrefix + ': 0,08 мешка на 1 кв.м. при финишном выравнивании. Для ' + area + ' кв.м. требуется около ' + bagQty(area * 0.08) + ' мешков.' },
    { name: 'Грунтовка глубокого проникновения', quantity: String(literQty(area * 0.12)), unit: 'л', unitPrice: external ? 180 : 140, environment: external ? 'external' : 'internal', sourceNote: sourcePrefix + ': 0,10-0,15 л на 1 кв.м. Для ' + area + ' кв.м. принято ' + literQty(area * 0.12) + ' л.' },
    { name: external ? 'Фасадная краска' : 'Водоэмульсионная краска для стен', quantity: String(literQty(area * 0.25)), unit: 'л', unitPrice: external ? 420 : 300, environment: external ? 'external' : 'internal', sourceNote: sourcePrefix + ': около 0,22-0,28 л на 1 кв.м. на 2 слоя. Для ' + area + ' кв.м. принято ' + literQty(area * 0.25) + ' л.' },
  ];
  return rows.map((row) => ({ ...row, total: Math.round(quantityNumber(row.quantity) * numeric(row.unitPrice)) }));
}

function shouldForceFinishingMaterials(input, materials) {
  if (!isAreaUnit(input?.unit) || !isFinishingWorkText([input?.workType, input?.repairGoal, input?.defectDescription].join(' '))) return false;
  if (!Array.isArray(materials) || !materials.length) return true;
  return materials.some((row) => {
    const name = String(row?.name || '').toLowerCase();
    const unit = String(row?.unit || '').toLowerCase();
    const qty = quantityNumber(row?.quantity);
    return /(штукатур|шпаклев|грунтов|краск)/i.test(name) && ((unit.includes('м3') || unit.includes('м³')) || qty < 0.5);
  });
}

function fallbackEstimate(input, error) {
  const plannedCost = Number(input.plannedCost || input.housePlanTotal || 0) || 100000;
  const amountText = String(input.amount || '').trim() || '1';
  const unitText = String(input.unit || '').trim() || 'ед.';
  const amountNumber = amountNumberFromInput(input);
  const workType = String(input.workType || input.repairGoal || 'Текущий ремонт');
  const lowerWork = workType.toLowerCase();
  const environment = input.workEnvironment && input.workEnvironment !== 'auto' ? input.workEnvironment : (/(фасад|кровл|двор|наруж|ступен|козыр|отмост)/i.test(workType) ? 'external' : 'internal');
  const materialsTarget = Math.round(plannedCost * 0.34);
  const laborTarget = Math.round(plannedCost * 0.48);
  const overhead = Math.round(plannedCost * 0.08);
  const profit = Math.max(Math.round((materialsTarget + laborTarget + overhead) * MIN_PROFIT_RATE), Math.round(plannedCost * MIN_PROFIT_RATE));
  const vat = Math.round((materialsTarget + laborTarget + overhead + profit) * VAT_RATE);
  const materialNames = lowerWork.includes('гвс') || lowerWork.includes('труб')
    ? ['Трубы и фитинги', 'Запорная арматура', 'Крепеж и расходные материалы']
    : lowerWork.includes('свет') || lowerWork.includes('элект')
      ? ['Светильники/электроматериалы', 'Кабель, крепеж, клеммы', 'Расходные материалы']
      : ['Шпаклевка/ремонтный состав', 'Грунтовка и подготовительные материалы', 'Финишное покрытие/краска и расходники'];
  const workNames = lowerWork.includes('гвс') || lowerWork.includes('труб')
    ? ['Демонтаж поврежденных участков', 'Монтаж трубопроводов и арматуры', 'Пусконаладка и проверка герметичности']
    : lowerWork.includes('свет') || lowerWork.includes('элект')
      ? ['Демонтаж неисправных элементов', 'Монтаж и подключение оборудования', 'Проверка работы и уборка зоны']
      : ['Подготовка поверхности', 'Основные ремонтно-отделочные работы', 'Финишная проверка и уборка'];
  const split = (total) => [Math.round(total * 0.45), Math.round(total * 0.35), total - Math.round(total * 0.45) - Math.round(total * 0.35)];
  const materialParts = split(materialsTarget);
  const workParts = split(laborTarget);
  let materials = materialNames.map((name, index) => {
    const total = materialParts[index] || 0;
    const quantity = index === 0 ? amountText : '1';
    const unit = index === 0 ? unitText : 'комплект';
    return {
      name,
      quantity,
      unit,
      unitPrice: quantity === '1' ? total : Math.round(total / amountNumber),
      total,
      environment,
      sourceNote: 'Шаблонная позиция: инженер должен сверить цену по региональным источникам или прайс-листам поставщиков.',
    };
  });
  if (isAreaUnit(input.unit) && isFinishingWorkText([input.workType, input.repairGoal, input.defectDescription].join(' '))) {
    materials = finishingMaterialDefaults(input, 'Шаблонный расчет по нормам расхода');
  }
  const works = workNames.map((name, index) => {
    const total = workParts[index] || 0;
    const quantity = index === 1 ? amountText : '1';
    const unit = index === 1 ? unitText : 'комплекс';
    const unitPrice = quantity === '1' ? total : Math.round(total / amountNumber);
    return {
      name,
      quantity,
      unit,
      maxUnitPrice: Math.round(unitPrice / LABOR_PRICE_FACTOR),
      unitPrice,
      total,
      environment,
      sourceNote: 'Работы поставлены по 80% от максимальной расчетной цены. Требуется проверка инженером.',
      visualExample: index === 1 ? 'Объем ' + amountText + ' ' + unitText + ' нужно сопоставить с реальным участком дома: подъезд, этажи, стояки или зона общего имущества.' : 'Служебная операция, без которой основной ремонт не будет надежным.',
    };
  });
  const finalMaterialsTotal = materials.reduce((sum, row) => sum + numeric(row.total), 0);
  const finalProfit = Math.max(Math.round((finalMaterialsTotal + laborTarget + overhead) * MIN_PROFIT_RATE), Math.round(plannedCost * MIN_PROFIT_RATE));
  const finalVat = Math.round((finalMaterialsTotal + laborTarget + overhead + finalProfit) * VAT_RATE);
  return {
    aiGenerated: false,
    setupRequired: !process.env.GEMINI_API_KEY,
    error: error || '',
    region: input.region || 'Киров',
    title: workType,
    assumptions: [
      'Смета сформирована в шаблонном режиме: региональные источники нужно проверить вручную на дату расчета: ' + currentPricingDateLabel() + '.',
      'Материалы и работы разделены отдельными таблицами, чтобы инженер мог менять объемы, цены и комментарии.',
      'Рентабельность не ниже 10% и НДС/налог 5% вынесены отдельными строками.',
    ],
    defects: input.defectDescription ? [{ area: input.address || 'Объект', observedDefect: input.defectDescription, likelyCause: 'Требует проверки инженером', consequence: 'Возможное ухудшение состояния общего имущества', requiredFix: input.repairGoal || workType, urgency: 'Проверить', confidence: 'низкая без AI-анализа фото' }] : [],
    materials,
    works,
    totals: {
      ...
      vat: finalVat,
      reserve: 0,
      total: finalMaterialsTotal + laborTarget + overhead + finalProfit + finalVat,
    },
    visualComparison: 'Для клиента объем ' + amountText + ' ' + unitText + ' нужно перевести в понятный пример: сколько это подъездов, этажей, светильников, стояков или типовых участков дома.',
    clientSummary: 'Предварительная стоимость работ по позиции "' + workType + '" сформирована как инженерный шаблон. Перед отправкой клиенту проверьте объемы, региональные цены, источники и применимость материалов.',
    engineerNotes: 'Проверьте фактический объем, единичные расценки, внутренний/наружный характер работ, источники цен, накладные расходы, рентабельность 10% и НДС/налог 5%.',
    sources: [],
  };
}
...
function buildPrompt(input) {
  const pricingDate = currentPricingDateLabel();
  return `Current calculation date: ${pricingDate}. Use only current prices as of ${pricingDate}. Do not write that prices are current for June 2024 or any past year. If exact publication dates are unknown, write: "Цены ориентировочные, проверены на дату расчета: ${pricingDate}."

РЎС„РѕСЂРјРёСЂСѓР№ РїСЂРµРґРІР°СЂРёС‚РµР»СЊРЅСѓСЋ СЃРјРµС‚Сѓ С‚РµРєСѓС‰РµРіРѕ СЂРµРјРѕРЅС‚Р° РјРЅРѕРіРѕРєРІР°СЂС‚РёСЂРЅРѕРіРѕ РґРѕРјР° РЅР° СЂСѓСЃСЃРєРѕРј СЏР·С‹РєРµ.
...
function normalizeEstimate(estimate) {
  ...
  const normalizedMaterials = materials.map((row) => {
    ...
    return { ...row, name: sanitizeCurrentDateText(row.name || ''), unit: sanitizeCurrentDateText(row.unit || ''), sourceNote: sanitizeCurrentDateText(row.sourceNote || ''), unitPrice: Math.round(numeric(row.unitPrice)), total: Math.round(total) };
  });
  ...
  const normalizedWorks = works.map((row) => {
    ...
    return {
      ...
      name: sanitizeCurrentDateText(row.name || ''),
      unit: sanitizeCurrentDateText(row.unit || ''),
      visualExample: sanitizeCurrentDateText(row.visualExample || ''),
      sourceNote: sanitizeCurrentDateText(row.sourceNote || `Цена работ рассчитана как 80% от максимальной региональной цены на дату расчета: ${currentPricingDateLabel()}.`),
    };
  });

  const estimateInput = estimate.input || estimate.request || {};
  const finalMaterials = shouldForceFinishingMaterials(estimateInput, normalizedMaterials)
    ? finishingMaterialDefaults(estimateInput, 'Защитный пересчет: исходные материалы не соответствовали указанной площади')
    : normalizedMaterials;
  const materialsTotal = finalMaterials.reduce((sum, row) => sum + numeric(row.total), 0);
  ...
  return {
    ...estimate,
    assumptions: Array.isArray(estimate.assumptions) ? estimate.assumptions.map(sanitizeCurrentDateText) : [],
    clientSummary: sanitizeCurrentDateText(estimate.clientSummary || ''),
    visualComparison: sanitizeCurrentDateText(estimate.visualComparison || ''),
    engineerNotes: sanitizeCurrentDateText(estimate.engineerNotes || ''),
    defects: defects.map((row) => ({
      area: sanitizeCurrentDateText(row.area || 'Объект'),
      observedDefect: sanitizeCurrentDateText(row.observedDefect || row.defect || 'Требует описания'),
      likelyCause: sanitizeCurrentDateText(row.likelyCause || row.cause || 'Уточнить'),
      consequence: sanitizeCurrentDateText(row.consequence || 'Уточнить риск'),
      requiredFix: sanitizeCurrentDateText(row.requiredFix || row.fix || 'Уточнить работы'),
