...
function safeFileName(value) {
  return String(value || 'file').replace(/[\\/:*?"<>|]+/g, '_').replace(/\s+/g, '_').slice(0, 120);
}
...
function ballotFields(payload, owner = {}) {
  return { address: payload.address || payload.house || '', startDate: payload.startDateText || dateRu(payload.startDate), endDate: payload.endDateText || dateRu(payload.endDate), ownerFlat: owner.flat || '', ownerArea: owner.area || owner.voteArea || owner.areaRaw || '', ownerName: owner.name || '', ownerId: owner.snils || owner.ownerId || '', ownerDocument: owner.document || owner.share || '' };
}
function noticeFields(payload) {
  return { address: payload.address || payload.house || '', startDate: payload.startDateText || dateRu(payload.startDate), paperEndDate: payload.paperEndDateText || payload.endDateText || dateRu(payload.endDate), endDate: payload.endDateText || dateRu(payload.endDate) };
}
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    if (payload.kind === 'notice') {
      ...
      response.setHeader('Content-Disposition', 'attachment; filename="' + safeFileName('soobshchenie_OSS_' + (payload.house || payload.address || 'dom')) + '.docx"');
    ...
    if (payload.kind === 'ballot') {
      ...
      response.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document');
      response.setHeader('Content-Disposition', 'attachment; filename="' + safeFileName('bulletin_OSS_' + (owner.flat || payload.house || 'client')) + '.docx"');
      response.status(200).send(buffer); return;
    }
    if (payload.kind === 'ballotsZip') {
      ...
      response.setHeader('Content-Disposition', 'attachment; filename="' + safeFileName('bulletins_OSS_' + (payload.house || payload.address || 'dom') + suffix) + '.zip"');
