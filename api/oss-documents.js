...
function fillDocx(templateName, fields, payload = {}) {
  ...
  Object.entries(fields).forEach(([key, value]) => {
    xml = xml.split('{{' + key + '}}').join(xmlEscape(value == null ? '' : String(value)));
  });
  if (fields.address) {
  ...
  if (fields.startDate) {
    xml = replaceVisibleText(xml, '{{startDate}}', fields.startDate);
    xml = replaceVisibleText(xml, '«20» апреля 2026', fields.startDate);
  }
  if (fields.paperEndDate) {
    xml = replaceVisibleText(xml, '{{paperEndDate}}', fields.paperEndDate);
    xml = replaceVisibleText(xml, '«11» июня 2026', fields.paperEndDate);
    xml = replaceVisibleText(xml, '29 мая 2026 г.', fields.paperEndDate + ' г.');
  }
  if (fields.endDate) {
    xml = replaceVisibleText(xml, '{{endDate}}', fields.endDate);
    xml = replaceVisibleText(xml, '«15» июня 2026', fields.endDate);
  }
