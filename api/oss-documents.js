...
function fillDocx(templateName, fields, payload = {}) {
  ...
  if (fields.address) {
    ...
    xml = replaceVisibleText(xml, 'Кировская обл, Киров, ул Дмитрия Козулева, д. 9', fields.address);
    xml = replaceVisibleRange(xml, 'расположенном по адресу:', 'Будет проводиться', 'расположенном по адресу: ' + fields.address + ' ');
    xml = replaceVisibleRange(xml, 'расположенном по адресу:', 'Собрание будет', 'расположенном по адресу: ' + fields.address + '. ');
  }
  if (fields.startDate) {
    xml = replaceVisibleRange(xml, 'Дата и время начала проведения заочного этапа общего собрания —', '(', 'Дата и время начала проведения заочного этапа общего собрания — ' + fields.startDate + ' ');
  }
  if (fields.paperEndDate) {
    xml = replaceVisibleRange(xml, 'Дата и время окончания приема решений на бумажном бюллетене собственников —', '(', 'Дата и время окончания приема решений на бумажном бюллетене собственников — ' + fields.paperEndDate + ' ');
  }
  if (fields.endDate) {
    xml = replaceVisibleRange(xml, 'Дата и время окончания приема решений собственников —', '(', 'Дата и время окончания приема решений собственников — ' + fields.endDate + ' ');
  }
  if (fields.questionsText && templateName.includes('notice')) {
