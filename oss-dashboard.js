(function () {
  ...
  function init() {
    ...
    document.addEventListener('click', (event) => {
      ...
      const presentationButton = event.target.closest('[data-presentation-id]');
      if (presentationButton) openRepairPresentation(Number(presentationButton.dataset.presentationId));
    });
  ...
  function renderRepairTable() {
    ...
    els.repairRegistryBody.innerHTML = rows.slice(0, 700).map((row) => {
      ...
      const presentation = row.budgetRisk ? `<button class="row-action" type="button" data-presentation-id="${row.id}">Презентация</button>` : '';
      return `<tr><td>${escapeHtml(row.complex)}</td><td>${escapeHtml(row.house || row.address)}</td><td>${escapeHtml(row.workType)}</td><td>${escapeHtml(row.deadlineRaw || '')}</td><td><span class="pill ${repairStatusClass(row)}">${escapeHtml(repairStatusLabel(row.statusKey))}</span></td><td>${formatMoney(row.plannedCost)}</td><td>${formatMoney(row.budget ? row.budget.annualBudget : 0)}</td><td>${risk}</td><td><button class="row-action" type="button" data-act-id="${row.id}">Акт</button> ${presentation}</td></tr>`;
    }).join('');
  }
  ...
  function openRepairPresentation(id) {
    ...
    const html = `<!doctype html><html lang="ru"><head><meta charset="utf-8"><title>Презентация для ОСС</title><style>@page{size:landscape;margin:0}body{margin:0;background:#eef3f8;color:#111;font-family:Arial,Helvetica,sans-serif}.toolbar{position:sticky;top:0;z-index:5;display:flex;flex-wrap:wrap;gap:8px;align-items:center;background:#fff;border-bottom:1px solid #d5dde7;padding:10px 14px}.toolbar button,.toolbar label{border:1px solid #9fb0c4;border-radius:6px;background:#f7fafc;padding:9px 12px;font-weight:700;cursor:pointer}.toolbar .primary{background:#e8f2ff;border-color:#6fa4dd}.toolbar input[type=file]{display:none}.deck{max-width:1280px;margin:18px auto 40px}.slide{position:relative;min-height:680px;margin:0 0 18px;background:#fff;border-radius:10px;box-shadow:0 4px 18px rgb(15 23 42 / .14);padding:48px;overflow:hidden}.slide h1{margin:0 0 24px;font-size:48px;line-height:1.05}.slide h2{margin:0 0 22px;font-size:36px}.slide p{font-size:22px;line-height:1.35}.accent{color:#e04412}.blue{color:#0878b8}.grid{display:grid;grid-template-columns:1fr 1fr;gap:34px}.cards{display:grid;grid-template-columns:repeat(3,1fr);gap:18px}.card{border:2px solid #dbe5ee;border-radius:14px;padding:20px;background:#f8fbff}.card strong{display:block;font-size:30px;margin-top:10px}.risk-card{border-color:#e04412;background:#fff4ef}.ok-card{border-color:#16823a;background:#f0fbf4}.photo-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-top:20px}.photo-grid img{width:100%;height:180px;object-fit:cover;border-radius:10px;border:1px solid #d9e0e8}.placeholder{display:flex;align-items:center;justify-content:center;height:180px;border:2px dashed #b8c4d2;border-radius:10px;color:#637083;background:#f8fafc}.calc{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-top:18px}.calc label{display:grid;gap:6px;font-weight:700;color:#526070}.calc input{min-height:42px;border:1px solid #c8d3df;border-radius:8px;padding:8px 10px;font:inherit}.big-number{font-size:74px;font-weight:900;color:#e04412}.decision{font-size:28px;font-weight:800}.footer{position:absolute;left:48px;right:48px;bottom:24px;display:flex;justify-content:space-between;color:#6b7788;font-size:14px}@media print{body{background:#fff}.toolbar{display:none}.deck{margin:0;max-width:none}.slide{box-shadow:none;border-radius:0;margin:0;page-break-after:always;min-height:100vh}}</style></head><body><div class="toolbar"><button class="primary" onclick="window.print()">Печать / PDF</button><label>Добавить фото дефектов<input id="photoInput" type="file" accept="image/*" multiple onchange="loadPhotos(event)"></label><button onclick="recalculate()">Пересчитать</button><button onclick="downloadPresentation()">Скачать HTML</button><button onclick="sendEmail()">Email</button><button onclick="sharePresentation()">Поделиться</button></div><main class="deck"><section class="slide"><h1>${escapeHtml(data.workType)}<br><span class="blue">${escapeHtml(data.house)}</span></h1><div class="grid"><div><h2 class="accent">Почему нужно вынести вопрос на ОСС?</h2><p>По дому недостаточно бюджета для выполнения работ. Чтобы провести ремонт, собственникам нужно принять решение на общем собрании и утвердить сбор средств.</p><p class="decision">Цель: согласовать работы, стоимость и порядок финансирования.</p></div><div class="card risk-card"><span>Дефицит бюджета</span><strong id="heroDeficit">${formatMoney(deficit)} руб.</strong><p>Красная зона означает, что план работ превышает доступный бюджет дома.</p></div></div><div class="footer"><span>ООО Сервисная компания "Азбука"</span><span>Материалы для ОСС</span></div></section><section class="slide"><h2>Фото дефектов и состояние объекта</h2><p>Инженер загружает фотографии дефектов перед отправкой презентации клиенту.</p><div id="photoGrid" class="photo-grid"><div class="placeholder">Фото 1</div><div class="placeholder">Фото 2</div><div class="placeholder">Фото 3</div></div><div class="footer"><span>${escapeHtml(data.address)}</span><span>Основание для работ</span></div></section><section class="slide"><h2>Объем и расчет стоимости</h2><div class="calc"><label>Объем<input id="amountInput" type="number" value="${data.amount || 1}" step="0.01"></label><label>Ед. изм.<input id="unitInput" value="${escapeAttr(data.unit || 'ед.')}"></label><label>Цена за ед., руб.<input id="priceInput" type="number" value="${Math.round(data.price || data.plannedCost || 0)}" step="0.01"></label><label>Месяцев сбора<input id="monthsInput" type="number" value="6" min="1"></label></div><div class="cards"><div class="card"><span>Бюджет дома</span><strong id="budgetValue">${formatMoney(budget)} руб.</strong></div><div class="card"><span>План работ</span><strong id="planValue">${formatMoney(row.housePlanTotal)} руб.</strong></div><div class="card risk-card"><span>Нужно собрать</span><strong id="deficitValue">${formatMoney(deficit)} руб.</strong></div></div><p id="monthlyText" class="decision"></p><div class="footer"><span>Предварительный расчет</span><span>Данные можно уточнить перед печатью</span></div></section><section class="slide"><h2>Предлагаемое решение для голосования</h2><p>1. Утвердить проведение работ: <strong>${escapeHtml(data.workType)}</strong>.</p><p>2. Утвердить стоимость работ и порядок сбора средств.</p><p>3. Поручить управляющей организации организовать выполнение работ после принятия решения собственниками.</p><p class="decision">Решение ОСС позволит провести ремонт планово, прозрачным способом и без аварийного сценария.</p><div class="footer"><span>${escapeHtml(data.address)}</span><span>Проект решения для ОСС</span></div></section></main><script>const repairData=${dataJson};function money(value){return Math.round(Number(value)||0).toLocaleString('ru-RU')}function recalculate(){const amount=Number(document.getElementById('amountInput').value)||0;const price=Number(document.getElementById('priceInput').value)||0;const months=Math.max(1,Number(document.getElementById('monthsInput').value)||1);const currentCost=amount*price;const plan=Math.max(currentCost,repairData.housePlanTotal||0);const deficit=Math.max(0,plan-(repairData.budget||0));document.getElementById('planValue').textContent=money(plan)+' руб.';document.getElementById('deficitValue').textContent=money(deficit)+' руб.';document.getElementById('heroDeficit').textContent=money(deficit)+' руб.';document.getElementById('monthlyText').textContent='Ориентир сбора: '+money(deficit/months)+' руб. в месяц на весь дом при рассрочке на '+months+' мес.'}function loadPhotos(event){const files=Array.from(event.target.files||[]).slice(0,6);const grid=document.getElementById('photoGrid');grid.innerHTML='';files.forEach((file)=>{const img=document.createElement('img');img.alt=file.name;img.src=URL.createObjectURL(file);grid.appendChild(img)});if(!files.length){grid.innerHTML='<div class="placeholder">Фото 1</div><div class="placeholder">Фото 2</div><div class="placeholder">Фото 3</div>'}}function cleanClone(){const clone=document.documentElement.cloneNode(true);clone.querySelectorAll('.toolbar,script').forEach((el)=>el.remove());return clone}function downloadPresentation(){const blob=new Blob(['<!doctype html>\\n'+cleanClone().outerHTML],{type:'text/html;charset=utf-8'});const link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download='presentation-oss-remont.html';link.click();setTimeout(()=>URL.revokeObjectURL(link.href),1000)}function sendEmail(){const subject='Презентация для ОСС: '+repairData.house+' - '+repairData.workType;const body='Добрый день. Направляем материалы для проведения ОСС по работам: '+repairData.workType+'. Дефицит бюджета: '+money(repairData.deficit)+' руб. Для отправки PDF нажмите Печать / PDF и приложите файл к письму.';location.href='mailto:?subject='+encodeURIComponent(subject)+'&body='+encodeURIComponent(body)}async function sharePresentation(){if(navigator.share){await navigator.share({title:document.title,text:'Материалы для ОСС: '+repairData.workType});return}await navigator.clipboard.writeText(document.body.innerText);alert('Текст презентации скопирован')}recalculate()</script></body></html>`;
