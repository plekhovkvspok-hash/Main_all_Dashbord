(function () {
  ...
  function findOssHeaderIndex(matrix) {
    return matrix.findIndex((row) => {
      ...
      return joined.includes('регион') && joined.includes('жк') && (joined.includes('мкд') || joined.includes('улица'));
    });
  }
  ...
  function quorumStatusClass(percent) {
    if (!percent) return '';
    if (percent >= 51) return 'status-quorum-green';
    if (percent >= 40) return 'status-quorum-orange';
    if (percent >= 25) return 'status-quorum-yellow';
    return 'status-quorum-red';
  }
