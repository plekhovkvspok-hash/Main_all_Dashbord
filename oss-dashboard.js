(function () {
  ...
  function renderRepairTable() {
    ...
    els.repairRegistryBody.innerHTML = rows.slice(0, 700).map((row) => {
      ...
      return `<tr><td>${escapeHtml(row.complex)}</td><td>${escapeHtml(row.house || row.address)}</td><td>${escapeHtml(row.workType)}</td><td>${escapeHtml(row.deadlineRaw || '')}</td><td><span class="pill ${repairStatusClass(row)}">${escapeHtml(repairStatusLabel(row.statusKey))}</span></td><td>${formatMoney(row.plannedCost)}</td><td>${formatMoney(row.budget ? row.budget.annualBudget : 0)}</td><td>${risk}</td><td><button class="row-action" type="button" data-act-id="${row.id}">Акт</button> ${presentation} ${aiPresentation} ${notebook}</td></tr>`;
    }).join('');
  }
...
`;
  ...
  async function openAiRepairPresentation(id) {
    ...
    try {
      ...
      showNotice(ai.aiGenerated
        ? 'AI-презентация сформирована через Gemini.'
        : 'AI-презентация открыта в шаблонном режиме. Для Gemini добавьте GEMINI_API_KEY в Vercel.');
    } catch (error) {
