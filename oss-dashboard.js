(function () {
  ...
  const sheetProxyUrl = '/.netlify/functions/sheet';
  ...
  function init() {
    ...
    els.loadGoogleBtn.addEventListener('click', loadFromGoogle);
  ...
  async function loadFromGoogle() {
    if (window.location.protocol !== 'file:') {
      ...
      try {
        const response = await fetch(sheetProxyUrl, { cache: 'no-store' });
