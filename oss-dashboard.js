(function () {
  ...
  async function fetchCsvPayload(apiUrl, fallbackUrl, force, fallbackTitle) {
    ...
    return { csv: await response.text(), sheetTitle: fallbackTitle || '' };
  }
  ...
  function wireRepairEstimateWindow(win, baseData) {
    ...
    function renderPhotos(message) {
      ...
      if (box) {
        ...
        box.innerHTML = photos.length ? photos.map((photo, index) => '<div class="photo-card"><img src="data:' + photo.mimeType + ';base64,' + photo.data + '" alt="' + escapeHtml(photo.name) + '"><small>Фото ДО ' + (index + 1) + '<br>' + escapeHtml(photo.name) + '</small></div>').join('') : 'После загрузки здесь появятся миниатюры фото дефектов ДО.';
      }
    ...
    function collect() {
      return {
        ...
        address: value('address'), workType: value('workType'), workEnvironment: value('workEnvironment'),
        amount: value('amount'), unit: value('unit'),
        plannedCost: Number(String(value('plannedCost')).replace(/\s/g, '').replace(',', '.')) || baseData.plannedCost,
        defectDescription: value('defectDescription'), repairGoal: value('repairGoal'), photos,
        priceSources: win.collectPriceSources ? win.collectPriceSources() : { region: value('region') || 'Киров', hasTrustedSources: false, links: [], files: [] },
      };
    }
    function fallbackRender(estimate) {
      if (win.renderEstimate) { win.currentEstimate = estimate; win.renderEstimate(estimate); return; }
      const result = doc.getElementById('result'); if (!result) return;
      const totals = estimate.totals || {};
      result.innerHTML = '<div class="notice"><strong>Смета сформирована.</strong><br>' + escapeHtml(estimate.clientSummary || '') + '</div>' + '<div class="summary"><div class="metric"><span>Материалы</span><strong>' + formatMoney(totals.materials || 0) + ' руб.</strong></div><div class="metric"><span>Работы</span><strong>' + formatMoney(totals.labor || 0) + ' руб.</strong></div><div class="metric"><span>Итого</span><strong>' + formatMoney(totals.total || 0) + ' руб.</strong></div></div>';
    }
    async function runEstimate(event) {
      if (event) event.preventDefault();
      setStatus('Ищу региональные цены и собираю смету...');
      try {
