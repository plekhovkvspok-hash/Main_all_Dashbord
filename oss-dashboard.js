(function () {
  ...
  const ossDocumentsApiUrl = '/api/oss-documents';
  const questionLibraryApiUrl = '/api/question-library';
  ...
  const defaultQuestionLibrary = [
    {
      ...
      questions: [
        'Утвердить выполнение работ: {workType} по адресу {address}.',
        'Утвердить сметную стоимость работ в размере {total} руб.',
        'Утвердить порядок финансирования работ клиентами пропорционально площади принадлежащих помещений.',
        'Уполномочить управляющую организацию организовать выполнение работ, контроль качества и оформление документов.',
      ],
    },
    {
      category: 'Пакет работ по дому',
      questions: [
        'Утвердить выполнение комплекса работ по текущему ремонту по адресу {address}: {works}.',
        'Утвердить предельную сметную стоимость комплекса работ в размере {total} руб.',
        'Утвердить порядок финансирования комплекса работ клиентами пропорционально площади принадлежащих помещений.',
        'Уполномочить управляющую организацию организовать выполнение комплекса работ, контроль качества и оформление документов.',
      ],
    },
    {
      category: 'ГИС ЖКХ / администратор',
      questions: [
        'Определить администратора общего собрания собственников помещений при проведении голосования с использованием ГИС ЖКХ.',
        'Утвердить порядок приема сообщений о проведении общих собраний собственников помещений в многоквартирном доме.',
        'Утвердить порядок приема решений собственников помещений по вопросам, поставленным на голосование.',
      ],
    },
    {
      category: 'Совет дома / полномочия',
      questions: [
        'Избрать совет многоквартирного дома.',
        'Утвердить полномочия совета многоквартирного дома.',
        'Наделить уполномоченное лицо правом подписывать акты приемки оказанных услуг и выполненных работ от имени собственников помещений.',
      ],
    },
    {
      category: '67% / общее имущество',
      questions: [
        'Принять решение об использовании общего имущества собственников помещений в многоквартирном доме.',
        'Принять решение об установке, размещении или демонтаже оборудования на общем имуществе многоквартирного дома.',
        'Уполномочить управляющую организацию организовать работы на общем имуществе и оформить необходимые документы.',
      ],
    },
  ];
  ...
  const state = {
    ...
    ossRows: [],
    ossFiltered: [],
    ossTableSort: { key: '', direction: 'asc' },
    expandedGanttMonths: new Set(),
    metricFilter: 'all',
    quorumFilter: '',
    riskPanelFilter: 'all',
    serviceRows: [],
    ...
    repairFactRows: [],
    repairFiltered: [],
    repairTableSort: { key: '', direction: 'asc' },
    repairMetricFilter: 'all',
    repairStatusFilter: '',
    expandedRepairHouses: new Set(),
    regionFilter: '',
  ...
  const els = {
    ...
    repairHouseFilter: document.getElementById('repairHouseFilter'),
    repairYearFilter: document.getElementById('repairYearFilter'),
    repairMonthFilter: document.getElementById('repairMonthFilter'),
    repairActDate: document.getElementById('repairActDate'),
    ...
    repairFactReportCount: document.getElementById('repairFactReportCount'),
    ...
    documentComplexSelect: document.getElementById('documentComplexSelect'),
    documentHouseSelect: document.getElementById('documentHouseSelect'),
    documentRepairSelect: document.getElementById('documentRepairSelect'),
    documentScopeSelect: document.getElementById('documentScopeSelect'),
    documentStartDate: document.getElementById('documentStartDate'),
    documentEndDate: document.getElementById('documentEndDate'),
    questionCategorySelect: document.getElementById('questionCategorySelect'),
    questionLibraryList: document.getElementById('questionLibraryList'),
    selectedQuestionsList: document.getElementById('selectedQuestionsList'),
    ...
    selectedQuestionsCount: document.getElementById('selectedQuestionsCount'),
    ownersFileInput: document.getElementById('ownersFileInput'),
    ballotTemplateInput: document.getElementById('ballotTemplateInput'),
    noticeTemplateInput: document.getElementById('noticeTemplateInput'),
    templateStatus: document.getElementById('templateStatus'),
    generateExactBallotsBtn: document.getElementById('generateExactBallotsBtn'),
    generateExactNoticeBtn: document.getElementById('generateExactNoticeBtn'),
    clearOwnersBtn: document.getElementById('clearOwnersBtn'),
    ownersRowsCount: document.getElementById('ownersRowsCount'),
    ownersValidCount: document.getElementById('ownersValidCount'),
    ownersErrorCount: document.getElementById('ownersErrorCount'),
    ownersAreaTotal: document.getElementById('ownersAreaTotal'),
  ...
  function init() {
    ...
    els.ossView.querySelectorAll('[data-oss-table-sort]').forEach((header) => {
      header.addEventListener('click', () => toggleOssTableSort(header.dataset.ossTableSort));
    });
    els.repairsView.querySelectorAll('[data-repair-table-sort]').forEach((header) => {
      header.addEventListener('click', () => toggleRepairTableSort(header.dataset.repairTableSort));
    });
    ...
    if (els.ownersFileInput) els.ownersFileInput.addEventListener('change', loadOwnersFromFile);
    if (els.ballotTemplateInput) els.ballotTemplateInput.addEventListener('change', (event) => loadOssTemplateFromFile(event, 'ballot'));
    if (els.noticeTemplateInput) els.noticeTemplateInput.addEventListener('change', (event) => loadOssTemplateFromFile(event, 'notice'));
    if (els.generateExactBallotsBtn) els.generateExactBallotsBtn.addEventListener('click', () => runRepairAction(els.generateExactBallotsBtn, openOssBallotsPdf, 'PDF бюллетени'));
    if (els.generateExactNoticeBtn) els.generateExactNoticeBtn.addEventListener('click', () => runRepairAction(els.generateExactNoticeBtn, openOssNoticePdf, 'PDF сообщение'));
    if (els.documentComplexSelect) els.documentComplexSelect.addEventListener('change', handleDocumentComplexChange);
    if (els.documentHouseSelect) els.documentHouseSelect.addEventListener('change', handleDocumentHouseChange);
    if (els.documentRepairSelect) els.documentRepairSelect.addEventListener('change', () => { renderTemplateStatus(); renderQuestionBuilder(); });
    if (els.documentScopeSelect) els.documentScopeSelect.addEventListener('change', renderQuestionBuilder);
    if (els.questionCategorySelect) els.questionCategorySelect.addEventListener('change', renderQuestionBuilder);
    if (els.addCustomQuestionBtn) els.addCustomQuestionBtn.addEventListener('click', addCustomDocumentQuestion);
    if (els.saveQuestionsBtn) els.saveQuestionsBtn.addEventListener('click', saveCurrentDocumentQuestions);
    if (els.resetQuestionsBtn) els.resetQuestionsBtn.addEventListener('click', resetCurrentDocumentQuestions);
    if (els.questionLibraryList) els.questionLibraryList.addEventListener('click', handleQuestionLibraryClick);
    if (els.selectedQuestionsList) {
      els.selectedQuestionsList.addEventListener('click', handleSelectedQuestionClick);
      els.selectedQuestionsList.addEventListener('input', handleSelectedQuestionInput);
    }
    if (els.clearOwnersBtn) els.clearOwnersBtn.addEventListener('click', clearOwnersRegistry);
    ...
    document.addEventListener('click', (event) => {
      ...
      const houseActButton = event.target.closest('[data-house-act-key]');
      if (houseActButton) {
        runRepairAction(houseActButton, () => openRepairHouseAct(houseActButton.dataset.houseActKey, houseActButton.dataset.actPeriod || 'period', houseActButton.dataset.actKind || 'plan'), 'Акт по дому');
        return;
      }
      const groupToggle = event.target.closest('[data-repair-group-toggle]');
      if (groupToggle) {
        toggleRepairHouseGroup(groupToggle.dataset.repairGroupToggle);
        return;
      }
    ...
    window.approveRepairEstimate = approveRepairEstimate;

    fillSelect(els.serviceNameFilter, serviceDefinitions.map((service) => service.label), 'Все услуги');
    if (els.repairActDate && !els.repairActDate.value) {
      els.repairActDate.value = new Date().toISOString().slice(0, 10);
    }
  ...
  async function runRepairAction(button, action, label) {
    ...
    button.setAttribute('aria-label', label + ': выполняется');
    ...
    try {
      ...
      console.error(error);
      showNotice((label || 'Действие') + ' не выполнено: ' + (error && error.message ? error.message : error));
  ...
  function loadApprovedEstimates() {
    try {
      const raw = localStorage.getItem('repair-approved-estimates');
  ...
  function saveApprovedEstimates() {
    try {
      localStorage.setItem('repair-approved-estimates', JSON.stringify(state.approvedEstimates || {}));
    } catch (error) {
      console.warn('Не удалось сохранить согласованные сметы', error);
    }
  }

  function approveRepairEstimate(id, estimate) {
    if (!id || !estimate) return;
    const normalized = { ...estimate, approvedAt: new Date().toISOString() };
    state.approvedEstimates[String(id)] = normalized;
    state.approvedEstimates[id] = normalized;
    saveApprovedEstimates();
    renderRepairTable();
    renderOssDocumentsModule();
  }
  ...
  async function loadAll(forceRefresh) {
    ...
    await Promise.allSettled([loadOss(force), loadServices(force), loadRepairs(force), loadQuestionLibraryFromGoogle(force)]);
    reconcileRepairComplexes();
    attachRepairHouseAreas();
    applyRepairFilters();
  ...
  async function loadOss(force) {
    const payload = await fetchCsvPayload(ossApiUrl, fallbackOssUrl, force, 'ОСС Киров');
    ingestOss(payload.csv, payload.sheetTitle);
  }

  async function loadServices(force) {
    const payload = await fetchCsvPayload(servicesApiUrl, null, force, 'АКТУАЛЬНО Тариф/Снег Киров');
    ingestServices(payload.csv, payload.sheetTitle);
  }
  ...
  async function loadRepairs(force) {
    ...
    ingestRepairs(payload.planCsv || '', payload.budgetCsv || '', payload.factCsv || '', payload.planSheetTitle || 'ТР Киров_план 26', payload.budgetSheetTitle || 'Баланс ТР Киров 2026', payload.factSheetTitle || 'Все работы 2026');
  }
  async function fetchCsvPayload(apiUrl, fallbackUrl, force, fallbackTitle) {
    const online = window.location.protocol !== 'file:';
    ...
    if (trimmed.startsWith('{')) {
      try {
        ...
        return { csv: payload.csv || '', sheetTitle: payload.sheetTitle || fallbackTitle || '' };
      } catch (error) {
        return { csv: text, sheetTitle: fallbackTitle || '' };
      }
    }
    return { csv: text, sheetTitle: fallbackTitle || '' };
  }
  ...
  function ingestOss(text, sheetTitle = 'ОСС Киров') {
    ...
    const rows = matrix.slice(headerIndex + 1)
      ...
      .map((row, index) => normalizeOssRecord(headers, row, index, sheetTitle))
  ...
  function normalizeOssRecord(headers, row, index, sheetTitle = '') {
    ...
    const plannedEndRaw = String(row[6] || '').trim() || get(/плановая дата завершения/, /план.*заверш/);
    const actualEndRaw = String(row[7] || '').trim() || get(/фактическая дата завершения/, /факт.*заверш/);
    const duration = toNumber(get(/срок голосования/));
    const startDate = parseDate(startRaw);
    const plannedEndDate = parseDate(plannedEndRaw) || addDays(startDate, duration || 30);
    const actualEndDate = parseDate(actualEndRaw);
    ...
    const record = {
      id: index + 1,
      region: detectRegion(get(/регион/), get(/город/), sheetTitle, object, get(/^жк$/)),
      regionMissing: !(get(/регион/) || sheetTitle),
      complex: get(/^жк$/),
      object,
  ...
  function renderGantt() {
    ...
    const columns = ganttColumns(from, addDays(from, days - 1));
    const grid = document.createElement('div');
    grid.className = 'gantt-grid gantt-grid-months';
    grid.style.setProperty('--days', columns.length);
    grid.style.setProperty('--gantt-columns', columns.map((column) => column.kind === 'month' || column.monthToggle ? '84px' : '22px').join(' '));
    ...
    columns.forEach((column) => {
      ...
      head.className = 'gantt-head' + (column.kind === 'month' || column.monthToggle ? ' gantt-month-head' : ' gantt-day-head');
      head.textContent = column.label;
      if (column.kind === 'month' || column.monthToggle) {
        if (column.monthToggle || (state.expandedGanttMonths && state.expandedGanttMonths.has(column.key))) head.classList.add('expanded');
        head.title = column.monthToggle ? 'Свернуть месяц' : 'Раскрыть месяц';
        head.dataset.monthKey = column.monthKey || column.key;
        head.addEventListener('click', () => toggleGanttMonth(column.monthKey || column.key));
      }
    ...
    rows.forEach((row) => {
      ...
      columns.forEach((column) => {
        ...
        cell.className = 'gantt-cell' + (column.kind === 'month' ? ' month-cell' : '') + (column.kind === 'day' && isWeekend(column.start) ? ' weekend' : '');
        if (dateRangesOverlap(column.start, column.end, row.startDate, row.endDate)) {
  ...
  function ganttColumns(from, to) {
    const columns = [];
    let cursor = startOfMonth(from);
    while (cursor <= to) {
      const start = cursor < from ? from : cursor;
      const rawEnd = endOfMonth(cursor);
      const end = rawEnd > to ? to : rawEnd;
      const key = monthKey(cursor);
      if (state.expandedGanttMonths && state.expandedGanttMonths.has(key)) {
        let first = true;
        for (let day = new Date(start); day <= end; day = addDays(day, 1)) {
          columns.push({
            kind: 'day',
            key: monthKey(day) + '-' + day.getDate(),
            monthKey: key,
            monthToggle: first,
            label: first ? formatShortMonth(day) + ' ' + String(day.getFullYear()).slice(2) : String(day.getDate()),
            start: day,
            end: day,
          });
          first = false;
        ...
        columns.push({ kind: 'month', key, label: formatShortMonth(cursor) + ' ' + String(cursor.getFullYear()).slice(2), start, end });
      }
  ...
  function toggleGanttMonth(key) {
    if (!state.expandedGanttMonths) state.expandedGanttMonths = new Set();
    if (state.expandedGanttMonths.has(key)) state.expandedGanttMonths.delete(key);
    else state.expandedGanttMonths.add(key);
    renderGantt();
  }
  ...
  function renderOssTable() {
    const rows = sortedOssTableRows(state.ossFiltered);
    els.rowsCount.textContent = `${rows.length} строк`;
    renderOssSortHeaders();
    els.registryBody.innerHTML = rows.slice(0, 500).map((row) => `<tr><td>${escapeHtml(row.complex)}</td><td>${escapeHtml(row.object)}</td><td>${ossPeriodHtml(row)}</td><td>${escapeHtml(row.type)}</td><td><span class="pill ${statusClass(row)}">${escapeHtml(row.quorumRaw ? `Кворум ${row.quorumRaw} / цель ${quorumTargetPercent(row)}%` : (row.status || `Цель ${quorumTargetPercent(row)}%`))}</span></td><td>${escapeHtml(row.questions)}</td><td>${formatNumber(row.planHours || estimateHours(row))}</td><td>${escapeHtml(row.owner)}</td><td>${row.risk.hasRisk ? escapeHtml(row.risk.reasons[0]) : ''}</td></tr>`).join('');
  }

  function ossPeriodHtml(row) {
    const period = [formatDate(row.startDate), formatDate(row.endDate)].filter(Boolean).join(' - ');
    const planned = row.actualEndDate && row.plannedEndDate ? '<br><span class="muted">план ' + escapeHtml(formatDate(row.plannedEndDate)) + '</span>' : '';
    return escapeHtml(period) + planned;
  }

  function toggleOssTableSort(key) {
    if (!key) return;
    const current = state.ossTableSort || {};
    state.ossTableSort = {
      key,
  ...
  function sortedOssTableRows(rows) {
    const sort = state.ossTableSort || {};
    if (!sort.key) return rows;
    const direction = sort.direction === 'desc' ? -1 : 1;
    return [...rows].sort((a, b) => compareOssSortValue(ossSortValue(a, sort.key), ossSortValue(b, sort.key)) * direction);
  }

  function ossSortValue(row, key) {
    if (key === 'complex') return row.complex || '';
    if (key === 'object') return row.object || '';
    if (key === 'period') return row.startDate ? row.startDate.getTime() : 0;
    if (key === 'type') return row.type || '';
    if (key === 'status') return row.quorumPercent || row.status || '';
    if (key === 'questions') return row.questions || '';
    if (key === 'hours') return Number(row.planHours || estimateHours(row) || 0);
    if (key === 'owner') return row.owner || '';
    if (key === 'risk') return row.risk && row.risk.hasRisk ? row.risk.reasons.join(' ') : '';
    return '';
  }

  function compareOssSortValue(a, b) {
    if (typeof a === 'number' || typeof b === 'number') return (Number(a) || 0) - (Number(b) || 0);
    return String(a || '').localeCompare(String(b || ''), 'ru', { numeric: true, sensitivity: 'base' });
  }

  function renderOssSortHeaders() {
    els.ossView.querySelectorAll('[data-oss-table-sort]').forEach((header) => {
      const active = state.ossTableSort && state.ossTableSort.key === header.dataset.ossTableSort;
      header.classList.toggle('is-sorted', !!active);
      header.dataset.sortDirection = active ? state.ossTableSort.direction : '';
      header.setAttribute('aria-sort', active ? (state.ossTableSort.direction === 'desc' ? 'descending' : 'ascending') : 'none');
    });
  }

  function ingestServices(text, sheetTitle = 'АКТУАЛЬНО Тариф/Снег Киров') {
    const matrix = parseCsv(text);
    ...
    const rows = matrix.slice(headerIndex + 1)
      ...
      .map((row, index) => normalizeServiceRecord(headers, row, index, sheetTitle))
  ...
  function normalizeServiceRecord(headers, row, index, sheetTitle = '') {
    const get = (...patterns) => getByHeader(headers, row, patterns);
    const services = {};
    ...
    return {
      id: index + 1,
      region: detectRegion(get(/регион/), get(/город/), sheetTitle, get(/^мкд$/, /адрес/), get(/^жк$/)),
      regionMissing: !(get(/регион/) || sheetTitle),
      complex: get(/^жк$/),
      object: get(/^мкд$/, /адрес/),
      area: toNumber(get(/площадь.*начислен/, /площадь/) || row[3]),
      currentTariff,
  ...
  function buildServiceAreaMap() {
    const map = new Map();
    state.serviceRows.forEach((row) => {
      const area = Number(row.area) || 0;
      if (!area) return;
      [
        normalizeRepairAddress(row.object),
        normalizeRepairAddress(row.address),
        normalizeRepairAddress((row.complex || "") + " " + (row.object || "")),
  ...
  function findServiceAreaForRepair(row, areaMap = buildServiceAreaMap()) {
    const keys = [
      normalizeRepairAddress(row.house),
      normalizeRepairAddress(row.address),
      normalizeRepairAddress((row.complex || "") + " " + (row.house || "")),
      normalizeRepairAddress(row.budget && row.budget.house),
    ].filter(Boolean);
    for (const key of keys) {
      if (areaMap.has(key)) return areaMap.get(key);
    }
    for (const key of keys) {
      for (const [candidate, area] of areaMap) {
        if (!candidate) continue;
        if (key.includes(candidate) || candidate.includes(key)) return area;
      }
    }
    return 0;
  }

  function attachRepairHouseAreas(rows = state.repairRows) {
    if (!rows || !rows.length || !state.serviceRows.length) return;
    const areaMap = buildServiceAreaMap();
    rows.forEach((row) => {
      const area = findServiceAreaForRepair(row, areaMap);
      row.houseArea = area || row.houseArea || 0;
      row.houseAreaMissing = !row.houseArea;
    });
  }
  ...
  function ingestRepairs(planText, budgetText, factText = '', planSheetTitle = 'ТР Киров_план 26', budgetSheetTitle = 'Баланс ТР Киров 2026', factSheetTitle = 'Все работы 2026') {
    const budgetRows = parseRepairBudget(budgetText, budgetSheetTitle);
    const factRows = parseRepairFacts(factText, factSheetTitle);
  ...
  function parseRepairFacts(text, sheetTitle = 'Все работы 2026') {
    const matrix = parseCsv(text);
    const headerIndex = matrix.findIndex((row) => {
      const joined = row.map(cleanHeader).join('|');
      return (joined.includes('адрес') || joined.includes('мкд') || joined.includes('дом')) && (joined.includes('вид работ') || joined.includes('работ'));
    });
    const legacy = parseLegacyRepairFacts(matrix, sheetTitle);
    if (headerIndex < 0) return legacy;
    const headers = matrix[headerIndex].map(cleanHeader);
    const parsed = matrix.slice(headerIndex + 1)
      .filter((row) => row.some((cell) => String(cell || '').trim()))
      .map((row, index) => normalizeRepairFactRecord(headers, row, index, sheetTitle))
      .filter((row) => row.address || row.workType);
    return legacy.length > parsed.length ? legacy : parsed;
  }

  function parseLegacyRepairFacts(matrix, sheetTitle = 'Все работы 2026') {
    return matrix
      .map((row, index) => normalizeLegacyRepairFactRecord(row, index, sheetTitle))
      .filter((row) => row.address && (row.workType || row.cost || row.actDate));
  }

  function normalizeLegacyRepairFactRecord(row, index, sheetTitle = '') {
    ...
    const workType = description || String(row[8] || row[6] || row[5] || '').trim();
    const actDateRaw = String(row[12] || '').trim();
    const cost = toNumber(row[14]);
    const amount = toNumber(row[15]) || 1;
    const status = 'Выполнено';
    const rowRegion = detectRegion(sheetTitle, address, complex);
    return {
      id: index + 1,
      address,
      complex,
      house,
      houseKey: normalizeRepairAddress(house || address),
      workType,
      workKey: normalizeRepairWork(workType),
      cost,
      amount,
      unit: 'ед.',
      actDateRaw,
      actDate: parseDate(actDateRaw),
      periodRaw: actDateRaw,
      status,
      region: rowRegion,
      note: [row[5], row[6], row[8], row[9], row[10]].filter(Boolean).join(' · '),
      sourceSheet: sheetTitle,
    };
  }

  function normalizeRepairFactRecord(headers, row, index, sheetTitle = '') {
    const get = (...patterns) => getByHeader(headers, row, patterns);
    const address = get(/^адрес$/, /^мкд$/, /^дом$/, /объект/);
    const complex = detectRepairComplex(address);
    const house = stripRepairComplex(address, complex);
    const workType = get(/вид работ/, /наименование.*работ/, /^работ/, /номенклат/, /материал/, /операц/, /услуг/, /описан/);
    const cost = toNumber(get(/сумма/, /стоим/, /цена/, /факт/));
    const amount = toNumber(get(/кол-?во/, /колич/, /объем/, /объ[её]м/));
    const unit = get(/ед\.?\s*изм/, /единиц/, /^ед\.?$/);
    const actDateRaw = get(/дата.*акт/, /дата.*выполн/, /дата.*закр/, /^дата$/);
    const periodRaw = get(/период/, /месяц/, /квартал/);
    const status = get(/статус/, /исполн/, /состояние/) || 'Выполнено';
    const rowRegion = detectRegion(get(/регион/), get(/город/), sheetTitle, address, complex);
    return {
      id: index + 1,
      address,
      complex,
      ...
      status,
      region: rowRegion,
      note: get(/примеч/, /коммент/),
      sourceSheet: sheetTitle,
    };
  }

  function attachRepairFacts(rows, facts) {
    if (!facts.length) return;
    const byHouseWork = new Map();
    facts.forEach((fact) => {
      const key = repairFactKey(fact.houseKey, fact.workKey);
      const current = byHouseWork.get(key) || [];
      current.push(fact);
      byHouseWork.set(key, current);
    });
    rows.forEach((row) => {
      const key = repairFactKey(row.houseKey || normalizeRepairAddress(row.house || row.address), normalizeRepairWork(row.workType));
      const matched = byHouseWork.get(key) || findSoftRepairFacts(row, facts);
      row.factRows = matched;
      row.factCost = matched.reduce((sum, fact) => sum + Number(fact.cost || 0), 0);
      row.factDate = matched.map((fact) => fact.actDate).filter(Boolean).sort((a, b) => b - a)[0] || null;
      row.factStatus = matched.length ? 'Выполнено по отчету' : '';
      if (matched.length && row.statusKey !== 'done') {
        row.status = row.status || 'Выполнено по отчету';
        row.statusKey = 'done';
      }
    });
  }

  function buildFactOnlyRepairRows(facts, planRows, budgets, sheetTitle = 'Все работы 2026') {
    const plannedFactKeys = new Set();
    planRows.forEach((row) => {
      if (!row.factRows || !row.factRows.length) return;
      row.factRows.forEach((fact) => plannedFactKeys.add(fact.id));
    });
    const budgetMap = new Map((budgets || []).map((budget) => [budget.key, budget]));
    return facts
      .filter((fact) => !plannedFactKeys.has(fact.id))
      .map((fact, index) => {
        const budget = findRepairBudget(fact.address, fact.house, budgetMap, budgets || [], fact.region);
        const actDate = fact.actDate || parseFactPeriodDate(fact.periodRaw) || new Date(2026, 0, 1);
        const row = {
          id: 900000 + index + 1,
          source: 'fact-only',
          sourceSheet: fact.sourceSheet || sheetTitle,
          region: fact.region || detectRegion(sheetTitle, fact.address, fact.complex),
          regionMissing: false,
          complex: fact.complex || detectRepairComplex(fact.address),
          address: fact.address || fact.house,
          house: fact.house || stripRepairComplex(fact.address, fact.complex),
          houseKey: fact.houseKey || normalizeRepairAddress(fact.house || fact.address),
          workType: fact.workType || 'Фактически выполненная работа',
          deadlineRaw: fact.actDate ? formatDate(fact.actDate) : fact.periodRaw || 'факт',
          status: 'Выполнено',
          statusKey: 'done',
          startDate: actDate,
          endDate: actDate,
          ...
          plannedCost: 0,
          factRows: [fact],
          factCost: Number(fact.cost || 0),
          factDate: fact.actDate || actDate,
          factStatus: fact.status || 'Выполнено',
          budget,
          budgetRisk: false,
          riskText: '',
        };
        row.budget = row.budget || findRepairBudget(row.address, row.house, budgetMap, budgets || [], row.region);
        return row;
      });
  }

  function parseFactPeriodDate(value) {
    ...
    const monthIndex = monthNames.findIndex((name) => raw.includes(name));
    if (monthIndex >= 0) return new Date(year, monthIndex, 1);
    if (/1\s*кв|i\s*кв|перв/.test(raw)) return new Date(year, 0, 1);
    if (/2\s*кв|ii\s*кв|втор/.test(raw)) return new Date(year, 3, 1);
    if (/3\s*кв|iii\s*кв|трет/.test(raw)) return new Date(year, 6, 1);
    if (/4\s*кв|iv\s*кв|четвер/.test(raw)) return new Date(year, 9, 1);
    return null;
  }

  function repairFactKey(houseKey, workKey) {
    return [houseKey || '', workKey || ''].join('|');
  }

  function normalizeRepairWork(value) {
    return normalizeText(value).replace(/\b(работы|работа|услуга|услуги|ремонт|текущий|текущии)\b/g, ' ').replace(/\s+/g, ' ').trim();
  }

  function findSoftRepairFacts(row, facts) {
    const houseKey = row.houseKey || normalizeRepairAddress(row.house || row.address);
    const workWords = new Set(normalizeRepairWork(row.workType).split(' ').filter((word) => word.length > 4));
    if (!houseKey || !workWords.size) return [];
    return facts.filter((fact) => {
      if (fact.houseKey !== houseKey) return false;
      const factWords = normalizeRepairWork(fact.workType).split(' ').filter((word) => word.length > 4);
      const overlap = factWords.filter((word) => workWords.has(word)).length;
      return overlap >= Math.min(2, workWords.size);
    });
  }

  function parseRepairBudget(text, sheetTitle = 'Баланс ТР Киров 2026') {
    const matrix = parseCsv(text);
    ...
    return matrix.slice(headerIndex + 1)
      ...
      .map((row, index) => {
        ...
        return {
          id: index + 1,
          house,
          region: detectRegion(get(/регион/), get(/город/), sheetTitle, house),
          regionMissing: !(get(/регион/) || sheetTitle),
  ...
  function parseRepairPlan(text, budgets, sheetTitle = 'ТР Киров_план 26') {
    const matrix = parseCsv(text);
    const headerIndex = matrix.findIndex((row) => {
      const joined = row.map(cleanHeader).join('|');
    ...
    const rows = matrix.slice(headerIndex + 1)
      .filter((row) => row.some((cell) => String(cell || '').trim()))
      .map((row, index) => normalizeRepairRecord(headers, row, index, budgetMap, budgets, sheetTitle))
      .filter((row) => row.address || row.workType);
    attachRepairBudgetTotals(rows);
    return rows;
  }

  function normalizeRepairRecord(headers, row, index, budgetMap, budgets, sheetTitle = '') {
    const get = (...patterns) => getByHeader(headers, row, patterns);
  ...
  function populateRepairDependentFilters(rows = state.repairRows) {
    if (!els.repairHouseFilter || !els.repairYearFilter || !els.repairMonthFilter) return;
    const selectedComplex = els.repairComplexFilter ? els.repairComplexFilter.value : '';
    const scoped = selectedComplex ? rows.filter((row) => row.complex === selectedComplex) : rows;
    const scopedFacts = selectedComplex ? state.repairFactRows.filter((row) => row.complex === selectedComplex) : state.repairFactRows;
    ...
    const yearValues = scoped
      ...
      .concat(scopedFacts.map((row) => row.actDate ? String(row.actDate.getFullYear()) : ''))
      .filter(Boolean);
    fillSelect(els.repairYearFilter, yearValues, 'Все годы');
    if (currentYear && Array.from(els.repairYearFilter.options).some((option) => option.value === currentYear)) els.repairYearFilter.value = currentYear;
    const monthValues = scoped
      .map((row) => row.startDate ? String(row.startDate.getMonth() + 1).padStart(2, '0') : '')
      .concat(scopedFacts.map((row) => row.actDate ? String(row.actDate.getMonth() + 1).padStart(2, '0') : ''))
      .filter(Boolean);
    fillSelect(els.repairMonthFilter, monthValues, 'Все месяцы');
    Array.from(els.repairMonthFilter.options).forEach((option) => {
      if (option.value) option.textContent = monthNameByNumber(option.value);
    });
    if (currentMonth && Array.from(els.repairMonthFilter.options).some((option) => option.value === currentMonth)) els.repairMonthFilter.value = currentMonth;
  }

  function applyRepairFilters() {
    populateRepairDependentFilters();
    const complex = els.repairComplexFilter.value;
    const house = els.repairHouseFilter ? els.repairHouseFilter.value : '';
    const year = els.repairYearFilter ? els.repairYearFilter.value : '';
    const month = els.repairMonthFilter ? els.repairMonthFilter.value : '';
    ...
    state.repairFiltered = state.repairRows.filter((row) => {
      ...
      if (month && (!periodDate || String(periodDate.getMonth() + 1).padStart(2, '0') !== month)) return false;
  ...
  function monthNameByNumber(value) {
    const names = ['январь', 'февраль', 'март', 'апрель', 'май', 'июнь', 'июль', 'август', 'сентябрь', 'октябрь', 'ноябрь', 'декабрь'];
    return names[Math.max(0, Math.min(11, Number(value) - 1))] || value;
  }

  function selectedRepairActDate() {
    if (els.repairActDate && els.repairActDate.value) {
      return formatDate(new Date(els.repairActDate.value));
    }
  ...
  function renderRepairMetrics() {
    ...
    const factReport = rows.filter((row) => row.source === 'fact-only' || (row.factRows && row.factRows.length)).length;
    ...
    if (els.repairFactReportCount) els.repairFactReportCount.textContent = factReport;
  ...
  function renderRepairGantt() {
    const rows = state.repairFiltered.filter((row) => repairGanttDate(row)).slice(0, 220);
    ...
    rows.forEach((row) => {
      ...
      months.forEach((month) => {
        ...
        const ganttDate = repairGanttDate(row);
        if (ganttDate && ganttDate.getMonth() === month.getMonth()) {
          const bar = document.createElement('div');
          bar.className = 'bar ' + repairStatusClass(row);
          bar.title = `${row.address}: ${row.workType}. ${repairStatusLabel(row.statusKey)}${row.factDate ? `. Факт: ${formatDate(row.factDate)}` : ''}`;
          cell.appendChild(bar);
        }
        grid.appendChild(cell);
      });
  ...
  function repairGanttDate(row) {
    if (row.statusKey === 'done' && row.factDate) return row.factDate;
    return row.startDate || row.factDate || null;
  }
  ...
  function renderRepairTable() {
    const rows = sortedRepairTableRows(state.repairFiltered);
    els.repairRowsCount.textContent = `${rows.length} строк`;
    renderRepairSortHeaders();
    els.repairRegistryBody.innerHTML = repairHouseGroups(rows).map((group) => {
      const expanded = state.expandedRepairHouses.has(group.key);
      const planned = group.rows.reduce((sum, row) => sum + row.plannedCost, 0);
      const fact = group.rows.reduce((sum, row) => sum + Number(row.factCost || 0), 0);
      const approvedTotal = group.rows.reduce((sum, row) => {
        const approved = state.approvedEstimates[row.id] || state.approvedEstimates[String(row.id)];
        return sum + (approved && approved.totals ? Number(approved.totals.total) || 0 : 0);
      }, 0);
      const riskCount = group.rows.filter((row) => row.budgetRisk).length;
      const groupRow = `<tr class="repair-group-row"><td colspan="10"><div class="repair-group-summary"><div class="repair-group-title"><button class="repair-group-toggle" type="button" data-repair-group-toggle="${escapeAttr(group.key)}">${expanded ? '−' : '+'}</button><strong>${escapeHtml(group.complex)} · ${escapeHtml(group.house)}</strong><span>${group.rows.length} работ · план ${formatMoney(planned)} руб.${fact ? ` · факт ${formatMoney(fact)} руб.` : ''}${approvedTotal ? ` · сметы ${formatMoney(approvedTotal)} руб.` : ''}${riskCount ? ` · риски ${riskCount}` : ''}</span></div><div class="repair-group-actions"><button class="row-action repair-group-act" type="button" data-house-act-key="${escapeAttr(group.key)}" data-act-period="month" data-act-kind="plan">План месяц</button><button class="row-action repair-group-act" type="button" data-house-act-key="${escapeAttr(group.key)}" data-act-period="month" data-act-kind="fact">Факт месяц</button><button class="row-action repair-group-act" type="button" data-house-act-key="${escapeAttr(group.key)}" data-act-period="month" data-act-kind="all">Все месяц</button><button class="row-action repair-group-act" type="button" data-house-act-key="${escapeAttr(group.key)}" data-act-period="year" data-act-kind="plan">План год</button><button class="row-action repair-group-act" type="button" data-house-act-key="${escapeAttr(group.key)}" data-act-period="year" data-act-kind="fact">Факт год</button><button class="row-action repair-group-act" type="button" data-house-act-key="${escapeAttr(group.key)}" data-act-period="year" data-act-kind="all">Все год</button></div></div></td></tr>`;
      return expanded ? groupRow + group.rows.slice(0, 120).map((row) => repairRowHtml(row)).join('') : groupRow;
    }).join('');
  }

  function repairRowHtml(row) {
    ...
    const approvedEstimate = state.approvedEstimates[row.id] || state.approvedEstimates[String(row.id)];
    const estimateTotal = approvedEstimate && approvedEstimate.totals ? Number(approvedEstimate.totals.total) || 0 : 0;
    const estimate = `<button class="row-action ${approvedEstimate ? 'approved-estimate' : ''}" type="button" data-estimate-id="${row.id}">${approvedEstimate ? '✓ Смета' : 'Смета'}</button>`;
    const factCell = row.factCost ? '<span class="ok-text">' + formatMoney(row.factCost) + '</span>' : '<span class="muted">нет факта</span>';
    return `<tr><td>${escapeHtml(row.complex)}${regionMark}</td><td>${escapeHtml(row.house || row.address)}${row.houseAreaMissing ? ' <span class="data-gap-chip warn">площадь?</span>' : ''}</td><td>${escapeHtml(row.workType)}</td><td>${escapeHtml(row.deadlineRaw || '')}${row.factDate ? '<br><span class="muted">факт ' + escapeHtml(formatDate(row.factDate)) + '</span>' : ''}</td><td><span class="pill ${repairStatusClass(row)}">${escapeHtml(repairStatusLabel(row.statusKey))}</span></td><td>${formatMoney(row.plannedCost)}${row.factCost ? '<br><span class="muted">факт ' + factCell + '</span>' : ''}</td><td>${estimateTotal ? formatMoney(estimateTotal) : '<span class="muted">не согласована</span>'}</td><td>${budgetCell}</td><td>${risk}</td><td><button class="row-action" type="button" data-act-id="${row.id}">Акт</button> ${estimate}</td></tr>`;
  }

  function repairHouseGroups(rows) {
    const map = new Map();
    rows.slice(0, 700).forEach((row) => {
      const key = row.houseKey || normalizeRepairAddress(row.house || row.address);
      const current = map.get(key) || { key, complex: row.complex || 'Без ЖК', house: row.house || row.address || 'Без дома', rows: [] };
      current.rows.push(row);
      map.set(key, current);
    });
    return Array.from(map.values()).sort((a, b) => `${a.complex} ${a.house}`.localeCompare(`${b.complex} ${b.house}`, 'ru', { numeric: true }));
  }

  function toggleRepairTableSort(key) {
    if (!key) return;
    const current = state.repairTableSort || {};
    state.repairTableSort = {
      key,
      direction: current.key === key && current.direction === 'asc' ? 'desc' : 'asc',
    };
    renderRepairTable();
  ...
  function sortedRepairTableRows(rows) {
    const sort = state.repairTableSort || {};
    if (!sort.key) return rows;
    const direction = sort.direction === 'desc' ? -1 : 1;
    return [...rows].sort((a, b) => compareRepairSortValue(repairSortValue(a, sort.key), repairSortValue(b, sort.key)) * direction);
  }

  function repairSortValue(row, key) {
    if (key === 'complex') return row.complex || '';
    if (key === 'house') return row.house || row.address || '';
    if (key === 'workType') return row.workType || '';
    if (key === 'period') return row.startDate ? row.startDate.getTime() : 0;
    if (key === 'status') return repairStatusLabel(row.statusKey);
    if (key === 'plannedCost') return Number(row.plannedCost || 0);
    if (key === 'estimateTotal') {
      const approved = state.approvedEstimates[row.id] || state.approvedEstimates[String(row.id)];
      return approved && approved.totals ? Number(approved.totals.total) || 0 : 0;
    }
    if (key === 'budget') return row.budget ? Number(row.budget.annualBudget || 0) : -1;
    if (key === 'risk') return row.riskText || (row.budgetRisk ? 'risk' : '');
    return '';
  }

  function compareRepairSortValue(a, b) {
    if (typeof a === 'number' || typeof b === 'number') return (Number(a) || 0) - (Number(b) || 0);
    return String(a || '').localeCompare(String(b || ''), 'ru', { numeric: true, sensitivity: 'base' });
  }

  function renderRepairSortHeaders() {
    els.repairsView.querySelectorAll('[data-repair-table-sort]').forEach((header) => {
      const active = state.repairTableSort && state.repairTableSort.key === header.dataset.repairTableSort;
      header.classList.toggle('is-sorted', !!active);
      header.dataset.sortDirection = active ? state.repairTableSort.direction : '';
      header.setAttribute('aria-sort', active ? (state.repairTableSort.direction === 'desc' ? 'descending' : 'ascending') : 'none');
    });
  }

  function toggleRepairHouseGroup(key) {
    if (!key) return;
    if (state.expandedRepairHouses.has(key)) state.expandedRepairHouses.delete(key);
    else state.expandedRepairHouses.add(key);
    renderRepairTable();
  }
  ...
  function repairHouseStats(rows) {
    ...
    rows.forEach((row) => {
      ...
      current.planned += row.plannedCost;
      current.fact += Number(row.factCost || 0);
      current.works += 1;
...
`;
  ...
  function wireRepairEstimateWindow(win, baseData) {
    const doc = win.document;
    let photos = [];
    let currentEstimate = null;
    let editMode = false;
    let estimateSigned = false;
    let signedEngineer = 'Инженер - Плехов Константин Валерьевич';
    const statusNode = () => doc.getElementById('status');
    const setStatus = (text) => { const node = statusNode(); if (node) node.textContent = text; };
    const value = (id) => (doc.getElementById(id) ? doc.getElementById(id).value : '');
    const setHtml = (id, html) => { const node = doc.getElementById(id); if (node) node.innerHTML = html; };
    const cleanNumber = (input) => Number(String(input || '').replace(/\s/g, '').replace(',', '.').replace(/[^\d.-]/g, '')) || 0;
    const qtyNumber = (input) => {
      const match = String(input || '').replace(',', '.').match(/-?\d+(\.\d+)?/);
      return match ? Number(match[0]) : 1;
    };
    const money = (input) => formatMoney(Math.round(cleanNumber(input)));
    function photoDataUrl(photo) { return photo && photo.data ? 'data:' + (photo.mimeType || 'image/jpeg') + ';base64,' + photo.data : ''; }
    function photoCardsHtml() {
      return photos.map((photo, index) => '<div class="photo-card"><img src="' + escapeHtml(photoDataUrl(photo)) + '" alt="' + escapeHtml(photo.name || ('Фото ' + (index + 1))) + '"><small>Фото ДО ' + (index + 1) + '<br>' + escapeHtml(photo.name || 'дефект') + '</small></div>').join('');
    }
    function renderPhotos(message) {
      const box = doc.getElementById('photos');
      const note = doc.getElementById('photoStatus');
      if (box) {
        box.className = photos.length ? 'photo-list' : 'photo-list empty';
        box.innerHTML = photos.length ? photoCardsHtml() : 'После загрузки здесь появятся миниатюры фото дефектов ДО.';
      }
      if (note) note.textContent = message || (photos.length ? 'Загружено фото дефектов ДО: ' + photos.length : 'Фото дефектов ДО еще не загружены.');
      win.photoPayload = photos;
    }
      ...
      return new Promise((resolve) => {
        ...
        reader.onload = () => {
          ...
          img.onload = () => {
            ...
            resolve({ name: file.name.replace(/\.[^.]+$/, '.jpg'), mimeType: 'image/jpeg', data: dataUrl.split(',')[1] || '' });
          };
    ...
    async function handlePhotos(event) {
      const files = Array.from(event.target.files || []).filter((file) => /^image\//.test(file.type)).slice(0, 8);
      photos = [];
      renderPhotos(files.length ? 'Сжимаю и загружаю фото дефектов ДО...' : 'Файлы не выбраны.');
      for (const file of files) {
        const photo = await fileToCompressedPhoto(file);
        if (photo && photo.data) { photos.push(photo); renderPhotos(); }
      }
      if (!photos.length) renderPhotos('Фото не загружены. Выберите изображения JPG/PNG/WebP.');
      event.target.value = '';
    }
    function collectPriceSourcesFallback() {
      if (win.collectPriceSources) return win.collectPriceSources();
      return { region: value('region') || 'Киров', hasTrustedSources: false, links: [], files: [] };
    }
    function collect() {
      return {
        ...baseData,
        region: value('region') || baseData.region || 'Киров',
        address: value('address') || baseData.address || '',
        workType: value('workType') || baseData.workType || '',
        workEnvironment: value('workEnvironment') || 'auto',
        amount: value('amount'),
        unit: value('unit'),
        plannedCost: cleanNumber(value('plannedCost')) || baseData.plannedCost,
        defectDescription: value('defectDescription'),
        repairGoal: value('repairGoal'),
        photos,
        priceSources: collectPriceSourcesFallback(),
      };
    }
    function envLabel(value) {
      if (value === 'internal') return 'Внутренние работы';
      if (value === 'external') return 'Наружные работы';
      if (value === 'mixed') return 'Смешанные работы';
      return 'Тип работ определяется по фото и описанию';
    }
    function cell(text, field, cls) {
      return '<td class="' + (cls || '') + '" data-field="' + field + '" contenteditable="' + (editMode ? 'true' : 'false') + '">' + escapeHtml(text == null ? '' : text) + '</td>';
    }
    function defectRowHtml(row) {
      return '<tr data-kind="defects">' + cell(row.area, 'area') + cell(row.observedDefect, 'observedDefect') + cell(row.likelyCause, 'likelyCause') + cell(row.consequence, 'consequence') + cell(row.requiredFix, 'requiredFix') + cell(row.urgency, 'urgency', 'urgency') + cell(row.confidence, 'confidence') + '<td class="actions"><button class="row-delete" onclick="deleteEstimateRow(this)">Удалить</button></td></tr>';
    }
    function materialRowHtml(row) {
      return '<tr data-kind="materials">' + cell(row.name, 'name', 'name') + cell(row.quantity, 'quantity', 'qty') + cell(row.unit, 'unit', 'unit') + cell(money(row.unitPrice), 'unitPrice', 'money') + cell(money(row.total), 'total', 'money') + cell(row.sourceNote, 'sourceNote', 'note') + '<td class="actions"><button class="row-delete" onclick="deleteEstimateRow(this)">Удалить</button></td></tr>';
    }
    function workRowHtml(row) {
      return '<tr data-kind="works">' + cell(row.name, 'name', 'name') + cell(row.quantity, 'quantity', 'qty') + cell(row.unit, 'unit', 'unit') + cell(money(row.unitPrice), 'unitPrice', 'money') + cell(money(row.total), 'total', 'money') + cell(row.visualExample || 'Уточнить понятный пример масштаба', 'visualExample', 'example') + cell(row.sourceNote, 'sourceNote', 'note') + '<td class="actions"><button class="row-delete" onclick="deleteEstimateRow(this)">Удалить</button></td></tr>';
    }
    function sourcesHtml(estimate) {
      const sources = Array.isArray(estimate.sources) ? estimate.sources : [];
      if (sources.length) return sources.map((source) => '<a href="' + escapeHtml(source.url || '#') + '" target="_blank" rel="noreferrer">' + escapeHtml(source.title || source.url || 'Источник') + '</a>').join('');
      const notes = [...(estimate.materials || []), ...(estimate.works || [])].map((row) => row.sourceNote).filter(Boolean).slice(0, 10);
      if (notes.length) return '<ul>' + notes.map((note) => '<li>' + escapeHtml(note) + '</li>').join('') + '</ul><p class="edit-hint">Gemini не вернул кликабельные ссылки, но оставил ценовые заметки по строкам. Проверьте их вручную перед отправкой клиенту.</p>';
      return 'Источники не вернулись. Проверьте расценки вручную.';
    }
    function normalizeEstimate(raw) {
      ...
      const materialTotal = materials.reduce((sum, row) => sum + cleanNumber(row.total || cleanNumber(row.unitPrice) * qtyNumber(row.quantity)), 0);
      const workTotal = works.reduce((sum, row) => sum + cleanNumber(row.total || cleanNumber(row.unitPrice) * qtyNumber(row.quantity)), 0);
      const sourceTotals = estimate.totals || {};
      const overhead = cleanNumber(sourceTotals.overhead || sourceTotals.delivery || 0);
      const reserve = cleanNumber(sourceTotals.reserve || 0);
      const profit = Math.max(Math.round((materialTotal + workTotal + overhead) * 0.1), cleanNumber(sourceTotals.profit));
      const vat = Math.max(Math.round((materialTotal + workTotal + overhead + profit + reserve) * 0.05), cleanNumber(sourceTotals.vat || sourceTotals.tax));
      return { ...estimate, defects, materials, works, totals: { ...sourceTotals, materials: materialTotal, labor: workTotal, overhead, reserve, profit, vat, total: materialTotal + workTotal + overhead + reserve + profit + vat } };
    }
    function renderEstimate(estimateInput) {
      currentEstimate = normalizeEstimate(estimateInput);
      win.currentEstimate = currentEstimate;
      const totals = currentEstimate.totals || {};
      const materials = currentEstimate.materials || [];
      const works = currentEstimate.works || [];
      const defects = currentEstimate.defects || [];
      const assumptions = Array.isArray(currentEstimate.assumptions) ? currentEstimate.assumptions : [];
      const result = doc.getElementById('result');
      if (!result) return;
      result.innerHTML = '<div class="estimate-brand"><div class="brand-left"><span class="brand-mark"></span><span><span class="brand-title">АЗБУКА</span><br><span class="brand-subtitle">СЕРВИСНАЯ КОМПАНИЯ</span></span></div><div class="doc-meta"><strong>СМЕТА</strong><br>' + escapeHtml(baseData.address || '') + '<br><span class="' + (estimateSigned ? 'signed-badge' : 'unsigned-badge') + '">' + (estimateSigned ? 'Проверено инженером' : 'Предварительный расчет') + '</span></div></div>'
        + '<h2>' + escapeHtml(currentEstimate.title || baseData.workType || 'Смета текущего ремонта') + '</h2>'
        + '<p>' + escapeHtml(currentEstimate.clientSummary || '') + '</p>'
        + '<div class="notice"><strong>Обязательные константы расчета.</strong><br>Работы считаются по 80% от максимальной найденной региональной цены. Рентабельность не ниже 10% и НДС/налог 5% вынесены отдельными строками.</div>'
        + '<div class="notice"><strong>Тип работ и ценообразование.</strong><br>' + escapeHtml(envLabel(value('workEnvironment') || currentEstimate.workEnvironment || 'auto')) + ': цены материалов и работ проверяются с учетом внутреннего/наружного характера работ.</div>'
        + '<div class="notice"><strong>Понятный масштаб для клиента.</strong><br>' + escapeHtml(currentEstimate.visualComparison || 'Добавьте пример масштаба: сколько это подъездов, этажей, светильников, стояков или типовых участков дома.') + '</div>'
        + '<div class="mini-actions"><button onclick="enableEstimateEdit()">Редактировать все ячейки</button><button onclick="recalculateEditedEstimate()">Пересчитать</button><button onclick="addEstimateRow(\'defects\')">Добавить дефект</button><button onclick="addEstimateRow(\'materials\')">Добавить материал</button><button onclick="addEstimateRow(\'works\')">Добавить работу</button></div><p class="edit-hint">После изменения объема, цены или суммы нажмите Enter либо кнопку Пересчитать. Все строки можно удалять и добавлять.</p>'
        + '<div class="table-section"><h3>Дефектовка по фото и описанию</h3><div class="table-wrap"><table class="table defect-table"><thead><tr><th>Зона</th><th>Дефект</th><th>Вероятная причина</th><th>Последствия</th><th>Что исправить</th><th>Срочность</th><th>Уверенность</th><th></th></tr></thead><tbody>' + (defects.map(defectRowHtml).join('') || '<tr><td class="empty-row" colspan="8">Загрузите фото дефектов и нажмите Рассчитать смету.</td></tr>') + '</tbody></table></div></div>'
        + '<div class="photo-history"><h3>Фото дефектов ДО</h3><div class="photo-list">' + (photos.length ? photoCardsHtml() : '<div class="empty-row">Фото дефектов ДО не загружены.</div>') + '</div></div>'
        + '<div class="table-section"><h3>Материалы</h3><div class="table-wrap"><table class="table material-table"><thead><tr><th>Материал</th><th>Кол-во</th><th>Ед.</th><th>Цена</th><th>Сумма</th><th>Источник/заметка</th><th></th></tr></thead><tbody>' + (materials.map(materialRowHtml).join('') || '<tr><td class="empty-row" colspan="7">Нет данных по материалам.</td></tr>') + '</tbody></table></div></div>'
        + '<div class="table-section"><h3>Работы</h3><div class="table-wrap"><table class="table work-table"><thead><tr><th>Работа</th><th>Кол-во</th><th>Ед.</th><th>Цена 80% от max</th><th>Сумма</th><th>Понятный пример</th><th>Источник/заметка</th><th></th></tr></thead><tbody>' + (works.map(workRowHtml).join('') || '<tr><td class="empty-row" colspan="8">Нет данных по работам.</td></tr>') + '</tbody></table></div></div>'
        + '<h3>Итог сметы</h3><div class="summary"><div class="metric"><span>Материалы</span><strong>' + money(totals.materials) + ' руб.</strong></div><div class="metric"><span>Работы</span><strong>' + money(totals.labor) + ' руб.</strong></div><div class="metric"><span>Накладные/доставка</span><strong>' + money(totals.overhead) + ' руб.</strong></div><div class="metric"><span>Сметная прибыль / рентабельность 10%</span><strong>' + money(totals.profit) + ' руб.</strong></div><div class="metric"><span>НДС/налог 5%</span><strong>' + money(totals.vat) + ' руб.</strong></div><div class="metric"><span>Итого</span><strong>' + money(totals.total) + ' руб.</strong></div></div>'
        + '<h3>Допущения</h3><ul>' + assumptions.map((item) => '<li>' + escapeHtml(item) + '</li>').join('') + '</ul>'
        + '<h3>Заметки инженеру</h3><p>' + escapeHtml(currentEstimate.engineerNotes || '') + '</p>'
        + '<div class="signature-block"><div><strong>Ответственный инженер</strong><div class="sign-line">' + escapeHtml(estimateSigned ? signedEngineer : 'Смета не подписана') + '</div><small>Проверил объемы, цены, источники и расчет</small></div><div><strong>Клиент / представитель дома</strong><div class="sign-line"></div><small>Ф.И.О., подпись</small></div></div>'
        + '<h3>Источники</h3><div class="sources">' + sourcesHtml(currentEstimate) + '</div>';
      if (editMode) enableEstimateEdit(true);
    }
    function textOf(row, field) {
      const cellNode = row.querySelector('[data-field="' + field + '"]');
      return cellNode ? cellNode.textContent.trim() : '';
    }
    function collectRows(kind) {
      return Array.from(doc.querySelectorAll('tr[data-kind="' + kind + '"]')).map((row) => {
        if (kind === 'defects') return { area: textOf(row, 'area'), observedDefect: textOf(row, 'observedDefect'), likelyCause: textOf(row, 'likelyCause'), consequence: textOf(row, 'consequence'), requiredFix: textOf(row, 'requiredFix'), urgency: textOf(row, 'urgency'), confidence: textOf(row, 'confidence') };
        const quantity = textOf(row, 'quantity');
        const unitPrice = cleanNumber(textOf(row, 'unitPrice'));
        const manualTotal = cleanNumber(textOf(row, 'total'));
        const calculatedTotal = Math.round(unitPrice * qtyNumber(quantity));
        const total = row.dataset.manualTotal === '1' ? manualTotal : calculatedTotal;
        return { name: textOf(row, 'name'), quantity, unit: textOf(row, 'unit'), unitPrice, total, visualExample: textOf(row, 'visualExample'), sourceNote: textOf(row, 'sourceNote') };
      }).filter((row) => Object.values(row).some(Boolean));
    }
    function bindEditEvents() {
      doc.querySelectorAll('tr[data-kind] [data-field]').forEach((node) => {
        node.oninput = () => {
          const row = node.closest('tr[data-kind]');
          if (!row) return;
          if (node.dataset.field === 'total') row.dataset.manualTotal = '1';
          if (node.dataset.field === 'quantity' || node.dataset.field === 'unitPrice') delete row.dataset.manualTotal;
        };
        node.onblur = () => {
          if (node.dataset.field === 'quantity' || node.dataset.field === 'unitPrice' || node.dataset.field === 'total') recalculateEditedEstimate(true);
        };
        node.onkeydown = (event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); node.blur(); } };
      });
    }
    function disableEstimateEdit(silent) {
      editMode = false;
      doc.querySelectorAll('tr[data-kind] [data-field]').forEach((node) => { node.contentEditable = 'false'; });
      doc.querySelectorAll('table').forEach((table) => table.classList.remove('editable-table'));
      if (!silent) setStatus('Режим правки выключен: пунктирные зоны убраны, смета готова к печати/PDF.');
    }
    function enableEstimateEdit(silent) {
      if (editMode && !silent) { disableEstimateEdit(false); return; }
      editMode = true;
      doc.querySelectorAll('tr[data-kind] [data-field]').forEach((node) => { node.contentEditable = 'true'; });
      doc.querySelectorAll('table').forEach((table) => table.classList.add('editable-table'));
      bindEditEvents();
      if (!silent) setStatus('Режим правки: можно менять любые ячейки, добавлять/удалять строки и пересчитывать смету. Повторное нажатие уберет пунктир.');
    }
    function recalculateEditedEstimate(silent) {
      if (!currentEstimate) return;
      const estimate = { ...currentEstimate, defects: collectRows('defects'), materials: collectRows('materials'), works: collectRows('works') };
      currentEstimate = normalizeEstimate(estimate);
      renderEstimate(currentEstimate);
      enableEstimateEdit(true);
      if (!silent) setStatus('Смета пересчитана: материалы, работы, рентабельность 10% и НДС 5% обновлены.');
    }
    function addEstimateRow(kind) {
      if (!currentEstimate) currentEstimate = normalizeEstimate({ defects: [], materials: [], works: [], totals: {} });
      if (kind === 'defects') currentEstimate.defects = [...(currentEstimate.defects || []), { area: 'Зона', observedDefect: 'Новый дефект', likelyCause: 'Уточнить', consequence: 'Уточнить', requiredFix: 'Уточнить работы', urgency: 'Планово', confidence: 'инженер' }];
      if (kind === 'materials') currentEstimate.materials = [...(currentEstimate.materials || []), { name: 'Материал', quantity: '1', unit: 'шт.', unitPrice: 0, total: 0, sourceNote: 'Добавлено инженером' }];
      if (kind === 'works') currentEstimate.works = [...(currentEstimate.works || []), { name: 'Работа', quantity: '1', unit: 'ед.', unitPrice: 0, total: 0, visualExample: 'Уточнить понятный пример', sourceNote: 'Добавлено инженером' }];
      renderEstimate(currentEstimate);
      enableEstimateEdit(true);
    }
    function deleteEstimateRow(button) {
      const row = button.closest('tr[data-kind]');
      if (row) row.remove();
      recalculateEditedEstimate(true);
    }
    function toggleEngineerTools() {
      const panel = doc.getElementById('engineerTools');
      if (panel) panel.classList.toggle('open');
    }
    function addEngineer() {
      const input = doc.getElementById('customEngineer');
      const select = doc.getElementById('engineerSelect');
    ...
    function signEstimate() {
      const select = doc.getElementById('engineerSelect');
      signedEngineer = select ? select.value : signedEngineer;
      estimateSigned = true;
      if (currentEstimate && baseData.id) approveRepairEstimate(baseData.id, currentEstimate);
      if (currentEstimate) renderEstimate(currentEstimate);
      setStatus('Смета подписана ответственным инженером: ' + signedEngineer);
    }
    function downloadEstimate() {
      disableEstimateEdit(true);
      const clone = doc.documentElement.cloneNode(true);
      clone.querySelectorAll('.toolbar,.form-panel,#notice,script').forEach((node) => node.remove());
      const blob = new win.Blob(['<!doctype html>\n' + clone.outerHTML], { type: 'text/html;charset=utf-8' });
      const link = doc.createElement('a');
      link.href = win.URL.createObjectURL(blob);
      link.download = 'smeta-tekushiy-remont.html';
      link.click();
      setTimeout(() => win.URL.revokeObjectURL(link.href), 1000);
    }
    function openPresentation() { if (baseData.id) openRepairPresentation(baseData.id, null, currentEstimate); }
    function openAct() { if (baseData.id) openRepairAct(baseData.id, currentEstimate); }
    function toggleSourceTools() {
      const panel = doc.getElementById('sourceTools');
      if (panel) panel.classList.toggle('open');
    }
    async function runEstimate(event) {
      if (event) event.preventDefault();
      setStatus('Ищу региональные цены и собираю подробную смету...');
      try {
        const response = await fetch(estimateApiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(collect()) });
        if (!response.ok) throw new Error('HTTP ' + response.status);
        const estimate = await response.json();
        editMode = false;
        renderEstimate(estimate);
        setStatus(estimate.aiGenerated ? 'Смета сформирована через Gemini с региональным поиском.' : 'Смета открыта в шаблонном режиме: ' + (estimate.error || 'нет доступа к Gemini/поиску'));
      } catch (error) {
        setStatus('Не удалось сформировать смету: ' + (error.message || error));
      }
    }
    const photoInput = doc.getElementById('photoInput'); if (photoInput) photoInput.onchange = handlePhotos;
    const button = doc.getElementById('estimateGenerateBtn'); if (button) { button.onclick = runEstimate; button.addEventListener('click', runEstimate); }
    win.generateEstimate = runEstimate;
    win.previewPhotos = handlePhotos;
    win.renderEstimate = renderEstimate;
    win.enableEstimateEdit = enableEstimateEdit;
    win.disableEstimateEdit = disableEstimateEdit;
    win.prepareEstimatePrint = () => { disableEstimateEdit(true); win.print(); };
    win.recalculateEditedEstimate = recalculateEditedEstimate;
    win.addEstimateRow = addEstimateRow;
    win.deleteEstimateRow = deleteEstimateRow;
    win.toggleEngineerTools = toggleEngineerTools;
    win.addEngineer = addEngineer;
    win.signEstimate = signEstimate;
    win.downloadEstimate = downloadEstimate;
    win.openPresentation = openPresentation;
    win.openAct = openAct;
    win.toggleSourceTools = toggleSourceTools;
    renderPhotos();
    if (baseData.approvedEstimate) {
      currentEstimate = baseData.approvedEstimate;
      estimateSigned = true;
      signedEngineer = currentEstimate.signedEngineer || signedEngineer;
      renderEstimate(currentEstimate);
      setStatus('Открыта согласованная инженером смета. Можно проверить, изменить и подписать заново.');
    }
  }
  ...
  function openRepairEstimate(id) {
    ...
    win.document.close();
    wireRepairEstimateWindow(win, data);
  }
  ...
  function openRepairPresentation(id, aiContent, estimatePayload) {
    const row = state.repairRows.find((item) => item.id === id);
    if (!row) return;
    const budget = row.budget ? row.budget.annualBudget : 0;
    const deficit = Math.max(0, row.housePlanTotal - budget);
    const data = {
      ...
      unit: row.unit || '',
      price: row.unitPrice || row.plannedCost,
      estimate: estimatePayload || state.approvedEstimates[id] || null,
      houseArea: row.houseArea || 0,
      houseAreaMissing: row.houseAreaMissing,
    };
    const estimate = estimatePayload || null;
    const estimateTotals = estimate && estimate.totals ? estimate.totals : {};
    const estimateMaterials = estimate && Array.isArray(estimate.materials) ? estimate.materials : [];
    const estimateWorks = estimate && Array.isArray(estimate.works) ? estimate.works : [];
    const estimateRows = (rows, title) => rows.length ? '<h3>' + title + '</h3><table class="payment-table estimate-table"><thead><tr><th>Позиция</th><th>Кол-во</th><th>Ед.</th><th>Цена</th><th>Сумма</th></tr></thead><tbody>' + rows.slice(0, 8).map((item) => '<tr><td>' + escapeHtml(item.name || '') + '</td><td>' + escapeHtml(item.quantity || '') + '</td><td>' + escapeHtml(item.unit || '') + '</td><td>' + formatMoney(item.unitPrice || 0) + ' руб.</td><td>' + formatMoney(item.total || 0) + ' руб.</td></tr>').join('') + '</tbody></table>' : '';
    const estimateSlide = estimate ? '<section id="estimateClientSection" class="slide"><div class="slide-title-spacer"><div class="brand-logo"><span class="brand-mark"></span><span><span class="brand-title">АЗБУКА</span><br><span class="brand-subtitle">СЕРВИСНАЯ КОМПАНИЯ</span></span></div></div><h2>Смета работ</h2><p class="payment-note">Предварительная смета прикладывается для прозрачного обсуждения стоимости на ОСС. Перед отправкой клиенту инженер проверяет объемы, цены и источники.</p><div class="cards"><div class="card"><span>Материалы</span><strong>' + formatMoney(estimateTotals.materials || 0) + ' руб.</strong></div><div class="card"><span>Работы</span><strong>' + formatMoney(estimateTotals.labor || 0) + ' руб.</strong></div><div class="card risk-card"><span>Итого</span><strong>' + formatMoney(estimateTotals.total || 0) + ' руб.</strong></div></div>' + estimateRows(estimateMaterials, 'Материалы') + estimateRows(estimateWorks, 'Работы') + '<div class="footer"><span>' + escapeHtml(data.address) + '</span><span>Смета к презентации</span></div></section>' : '';
    const estimateToolbar = estimate ? '<label class="client-toggle"><input id="showEstimateClient" type="checkbox" checked onchange="toggleEstimateForClient()"> Показать смету клиенту</label>' : '';
    const ai = aiContent || {};
  ...
  function openRepairAct(id, estimatePayload) {
    const row = state.repairRows.find((item) => item.id === id);
    if (!row) return;
    const date = selectedRepairActDate();
    const address = row.address || row.house || '';
    const workName = row.workType || '';
    const quantity = row.amount ? formatNumber(row.amount) : '';
    const estimate = estimatePayload || state.approvedEstimates[id] || state.approvedEstimates[String(id)] || null;
    const estimateTotals = estimate && estimate.totals ? estimate.totals : {};
    const actTotalNumber = repairActPrice(row, estimate);
    const price = formatMoney(actTotalNumber);
    const total = formatMoney(actTotalNumber);
    const estimateRowsHtml = (title, rows) => Array.isArray(rows) && rows.length ? '<p class="paragraph"><strong>' + title + '</strong></p><table class="work-table estimate-attachment"><thead><tr><th>Позиция</th><th class="qty">Кол-во</th><th class="qty">Ед.</th><th class="money">Цена</th><th class="money">Сумма</th></tr></thead><tbody>' + rows.map((item) => '<tr><td>' + escapeHtml(item.name || '') + '</td><td class="qty">' + escapeHtml(item.quantity || '') + '</td><td class="qty">' + escapeHtml(item.unit || '') + '</td><td class="money">' + formatMoney(item.unitPrice || 0) + '</td><td class="money">' + formatMoney(item.total || 0) + '</td></tr>').join('') + '</tbody></table>' : '';
    const estimateAttachment = estimate ? '<section id="actEstimateSection"><p class="paragraph"><strong>Приложение: смета к акту</strong></p><p class="paragraph">Смета включена в акт для прозрачного подтверждения состава работ и стоимости.</p>' + estimateRowsHtml('Материалы', estimate.materials) + estimateRowsHtml('Работы', estimate.works) + '<table class="work-table"><tbody><tr><td>Материалы</td><td class="money">' + formatMoney(estimateTotals.materials || 0) + '</td></tr><tr><td>Работы</td><td class="money">' + formatMoney(estimateTotals.labor || 0) + '</td></tr><tr><td>Рентабельность 10%</td><td class="money">' + formatMoney(estimateTotals.profit || 0) + '</td></tr><tr><td>НДС/налог 5%</td><td class="money">' + formatMoney(estimateTotals.vat || 0) + '</td></tr><tr class="total-row"><td>Итого по смете</td><td class="money">' + formatMoney(estimateTotals.total || 0) + '</td></tr></tbody></table></section>' : '';
    const estimateActToolbar = estimate ? '<label class="client-toggle"><input id="showEstimateClient" type="checkbox" checked onchange="toggleEstimateForClient()"> Показать смету клиенту</label>' : '';
...
приказом Министерства строительства и жилищно-коммунального хозяйства Российской Федерации от 26.10.2015 № 761/пр</div><div class="title center">АКТ</div><div class="subtitle center">приемки оказанных услуг и выполненных работ</div><div class="subtitle center">по текущему ремонту общего имущества в многоквартирном доме</div><div class="spacer"></div><div class="line-row"><div>г. Киров</div><div>дата приемки работ:</div><input class="act-field" data-field="actDate" value="${escapeAttr(date)}" style="width:33mm"></div><p class="paragraph">Собственники помещений в многоквартирном доме, расположенном по адресу:</p><p class="paragraph"><strong>${escapeHtml(address)}</strong></p><p class="paragraph">именуемые в дальнейшем “Заказчик”, в лице <input class="act-field" data-field="customerName" style="width:78mm"></p><p class="paragraph">являющегося собственником квартиры № <input class="act-field" data-field="flatNumber" style="width:22mm">, находящейся в данном многоквартирном доме, действующего на основании протокола ОСС № <input class="act-field" data-field="protocolNumber" style="width:50mm"></p><p class="paragraph">с одной стороны, и <span id="organizationName">ООО Сервисная компания "Азбука"</span>, именуемое в дальнейшем “Исполнитель”, в лице</p><p class="paragraph">исполнительного директора Плехова Константина Валерьевича, действующего на основании приказа № 2 от 01.02.2022г., с другой стороны, совместно именуемые “Стороны”, составили настоящий Акт о нижеследующем:</p><p class="paragraph indent">1. Исполнителем предъявлены к приемке следующие оказанные на основании договора управления многоквартирным домом № <input class="act-field" data-field="contractNumber" style="width:30mm"> от <input class="act-field" data-field="contractDate" style="width:24mm"> г., <strong>${escapeHtml(address)}</strong> (далее - "Договор") услуги и выполненные работы по текущему ремонту общего имущества в вышеуказанном многоквартирном доме:</p><table class="work-table"><thead><tr><th class="num">№п</th><th>Наименование вида работы (услуги)</th><th class="period">Периодичность выполненной работы (оказанной услуги)</th><th class="qty">Количественный показатель выполненной работы (оказанной услуги)</th><th class="money">Цена выполненной работы (оказанной услуги), в рублях</th><th class="money">Стоимость/сметная стоимость выполненной работы (оказанной услуги) за единицу</th></tr></thead><tbody><tr><td class="num">1.</td><td>${escapeHtml(workName)}</td><td>${escapeHtml(row.deadlineRaw || '')}</td><td class="qty">${escapeHtml(quantity)}${row.unit ? ` ${escapeHtml(row.unit)}` : ''}</td><td class="money">${escapeHtml(price)}</td><td class="money">${escapeHtml(total)}</td></tr><tr class="total-row"><td></td><td>ИТОГО</td><td></td><td></td><td></td><td class="money">${escapeHtml(total)}</td></tr></tbody></table><div class="sum-line"><span>2. Всего выполнено работ (оказано услуг) на общую сумму</span><span class="fill"></span><span class="amount">${escapeHtml(total)}</span><span>руб.</span></div>${estimateAttachment}<p class="paragraph">3. Работы (услуги) выполнены (оказаны) полностью, в установленные сроки, с надлежащим качеством.</p><p class="paragraph">4. Претензий по выполнению условий Договора Стороны друг к другу не имеют.</p><p class="paragraph">Настоящий Акт составлен в 2-х экземплярах, имеющих одинаковую юридическую силу, по одному для каждой из Сторон.</p><p class="signature-title">Подписи Сторон:</p><div class="sign-grid"><div><p>Исполнитель -</p><div id="signaturePlace" class="sign-line"></div><div id="signatureName" class="signature-name"></div><div id="signatureFacsimile" class="signature-facsimile"></div><div class="sign-caption"><span>(Ф.И.О.)</span><span>(подпись)</span></div></div><div><p>Заказчик -</p><div class="sign-line"></div><div class="sign-caption"><span>(Ф.И.О.)</span><span>(подпись)</span></div></div></div><div id="sealStamp" class="seal">ООО СК<br>АЗБУКА<br>КИРОВ</div></main><script>const actTitle=document.title;const storageKey="repair-act-fields:"+${JSON.stringify(address)};function selectedEmployee(){const select=document.getElementById("employeeSelect");return select&&select.value?select.value:"Исполнительный директор — Плехов Константин Валерьевич"}function selectedOrg(){const custom=document.getElementById("customOrg");const select=document.getElementById("orgSelect");return custom&&custom.value.trim()?custom.value.trim():(select?select.value:"ООО Сервисная компания \\\"Азбука\\\"")}function updateOrganization(){const org=selectedOrg();document.getElementById("organizationName").textContent=org;const seal=document.getElementById("sealStamp");if(!seal.querySelector("img")){seal.innerHTML=org.replace(/ООО|Общество с ограниченной ответственностью|[\\"«»]/g,"").trim().replace(/\\s+/g,"<br>")+"<br>КИРОВ"}saveActFields()}function loadSeal(event){const file=event.target.files&&event.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{const seal=document.getElementById("sealStamp");seal.innerHTML="<img alt=\\"Оттиск печати\\" src=\\""+reader.result+"\\">";seal.classList.add("visible")};reader.readAsDataURL(file)}function saveActFields(){const data={};document.querySelectorAll(".act-field").forEach((field)=>data[field.dataset.field]=field.value);data.organization=selectedOrg();localStorage.setItem(storageKey,JSON.stringify(data))}function restoreActFields(){try{const data=JSON.parse(localStorage.getItem(storageKey)||"{}");document.querySelectorAll(".act-field").forEach((field)=>{if(data[field.dataset.field])field.value=data[field.dataset.field]});if(data.organization){document.getElementById("customOrg").value=data.organization}updateOrganization()}catch(error){}}function toggleSignTools(){document.getElementById("signTools").classList.toggle("open")}document.querySelectorAll(".act-field").forEach((field)=>field.addEventListener("input",saveActFields));restoreActFields();function applySignature(){const name=selectedEmployee();document.getElementById("signatureName").textContent=name;document.getElementById("signatureFacsimile").textContent=name.split(" ").map((part,index)=>index===0?part:part[0]+".").join(" ")}function applySeal(){document.getElementById("sealStamp").classList.toggle("visible")}function toggleEstimateForClient(){const section=document.getElementById('actEstimateSection');const input=document.getElementById('showEstimateClient');if(section&&input)section.classList.toggle('hidden-for-client',!input.checked)}function actText(){return document.querySelector('.sheet').innerText.trim()}function downloadAct(){const clone=document.documentElement.cloneNode(true);clone.querySelectorAll('.toolbar,script').forEach((el)=>el.remove());const blob=new Blob(['<!doctype html>\\n'+clone.outerHTML],{type:'text/html;charset=utf-8'});const link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download='akt-tekushiy-remont.html';link.click();setTimeout(()=>URL.revokeObjectURL(link.href),1000)}function sendEmail(){const body=actText()+'\\n\\nДля отправки подписанного акта сохраните форму в PDF и прикрепите файл к письму.';location.href='mailto:?subject='+encodeURIComponent(actTitle)+'&body='+encodeURIComponent(body.slice(0,1800))}async function shareAct(){if(navigator.share){await navigator.share({title:actTitle,text:actText()});return}copyActText()}async function copyActText(){await navigator.clipboard.writeText(actText());alert('Текст акта скопирован')}</script></body></html>`;
  ...
  function openRepairHouseAct(groupKey, periodMode = 'period', actKind = 'plan') {
    const year = els.repairYearFilter ? els.repairYearFilter.value : '';
    const month = els.repairMonthFilter ? els.repairMonthFilter.value : '';
    if (periodMode === 'year' && !year) {
      showNotice('Для годового акта выберите год в фильтре текущего ремонта.');
      return;
    }
    if (periodMode === 'month' && (!year || !month)) {
      showNotice('Для акта за месяц выберите год и месяц в фильтрах текущего ремонта.');
      return;
    }
    const housePlanRows = state.repairRows
      .filter((row) => (row.houseKey || normalizeRepairAddress(row.house || row.address)) === groupKey)
    const planRows = housePlanRows
      .filter((row) => repairActRowInPeriod(row.startDate, row.year, periodMode, year, month))
      .map((row) => repairPlanActRow(row));
    const reportFactRows = state.repairFactRows
      .filter((row) => row.houseKey === groupKey)
      .filter((row) => repairActRowInPeriod(row.actDate, '', periodMode, year, month))
      .map((row) => repairFactActRow(row));
    const completedPlanFactRows = housePlanRows
      .filter((row) => row.statusKey === 'done')
      .filter((row) => repairActRowInPeriod(row.factDate || row.startDate, row.year, periodMode, year, month))
      .map((row) => repairCompletedPlanFactActRow(row));
    const factRows = dedupeRepairActRows(reportFactRows.concat(completedPlanFactRows));
    const rows = actKind === 'fact' ? factRows : actKind === 'all' ? planRows.concat(factRows) : planRows;
    if (!rows.length) {
      showNotice('Нет ' + repairActKindLabel(actKind).toLowerCase() + ' работ для акта по выбранному дому и периоду.');
      return;
    }
    const first = rows[0];
    const date = selectedRepairActDate();
    const period = repairActPeriodLabel(rows, periodMode);
    const total = rows.reduce((sum, row) => sum + Number(row.actPrice || 0), 0);
    const rowsHtml = rows.map((row, index) => {
      const price = Number(row.actPrice || 0);
      return `<tr><td class="num">${index + 1}</td><td>${escapeHtml(row.workType || '')}</td><td>${escapeHtml(row.actPeriod || row.deadlineRaw || '')}</td><td class="qty">${row.amount ? escapeHtml(formatNumber(row.amount)) : ''}${row.unit ? ' ' + escapeHtml(row.unit) : ''}</td><td class="money">${formatMoney(price)}</td><td class="money">${formatMoney(price)}</td></tr>`;
    }).join('');
    const html = `<!doctype html><html lang="ru"><head><meta charset="utf-8"><title>Акт по дому</title><style>@page{size:A4;margin:14mm 12mm}body{margin:0;background:#f3f5f7;color:#000;font-family:"Times New Roman",Times,serif;font-size:13px;line-height:1.25}.toolbar{display:flex;gap:8px;background:#fff;border-bottom:1px solid #d0d7de;padding:10px 14px;font-family:Arial,sans-serif}.toolbar button{border:1px solid #9aa7b4;border-radius:6px;background:#f5f7f9;padding:9px 12px;font:700 14px Arial,sans-serif;cursor:pointer}.sheet{width:190mm;min-height:267mm;margin:18px auto;background:#fff;padding:8mm;box-shadow:0 2px 12px rgb(15 23 42 / .12)}.center{text-align:center}.title{font-size:17px;font-weight:700;margin-top:18mm}.paragraph{margin:8px 0}.work-table{width:100%;border-collapse:collapse;margin:14px 0 8px;font-size:11px}.work-table th,.work-table td{border:1px solid #000;padding:5px 4px;vertical-align:middle}.work-table th{text-align:center;font-weight:400}.num{width:10mm;text-align:center}.qty{width:28mm;text-align:center}.money{width:32mm;text-align:right}.total-row td{font-weight:700}.sign-grid{display:grid;grid-template-columns:1fr 1fr;gap:18mm;margin-top:24px}.sign-line{height:22px;border-bottom:1px solid #000}@media print{body{background:#fff}.toolbar{display:none}.sheet{margin:0;box-shadow:none;width:auto;min-height:auto;padding:0}}</style></head><body><div class="toolbar"><button onclick="window.print()">Печать / PDF</button><button onclick="downloadAct()">Скачать HTML</button></div><main class="sheet"><div class="title center">АКТ</div><div class="center">приемки оказанных услуг и выполненных работ по текущему ремонту</div><p class="paragraph">г. Киров <span style="float:right">дата приемки работ: <input class="act-field" value="${escapeAttr(date)}" style="width:32mm"></span></p><p class="paragraph">Адрес МКД: <strong>${escapeHtml(first.address || first.house || '')}</strong></p><p class="paragraph">Тип выгрузки: <strong>${escapeHtml(repairActKindLabel(actKind))}</strong></p><p class="paragraph">Период работ: <strong>${escapeHtml(period)}</strong></p><p class="paragraph">Исполнителем предъявлены к приемке следующие услуги и выполненные работы по текущему ремонту общего имущества:</p><table class="work-table"><thead><tr><th class="num">№п</th><th>Наименование вида работы (услуги)</th><th>Периодичность выполненной работы (оказанной услуги)</th><th class="qty">Количественный показатель</th><th class="money">Цена, руб.</th><th class="money">Стоимость, руб.</th></tr></thead><tbody>${rowsHtml}<tr class="total-row"><td></td><td>ИТОГО</td><td></td><td></td><td></td><td class="money">${formatMoney(total)}</td></tr></tbody></table><p class="paragraph">Всего выполнено работ (оказано услуг) на общую сумму <strong>${formatMoney(total)}</strong> руб.</p><p class="paragraph">Работы (услуги) выполнены (оказаны) полностью, в установленные сроки, с надлежащим качеством.</p><div class="sign-grid"><div><p>Исполнитель -</p><div class="sign-line"></div><p class="center">(Ф.И.О., подпись)</p></div><div><p>Заказчик -</p><div class="sign-line"></div><p class="center">(Ф.И.О., подпись)</p></div></div></main><script>function downloadAct(){const clone=document.documentElement.cloneNode(true);clone.querySelectorAll('.toolbar,script').forEach((el)=>el.remove());const blob=new Blob(['<!doctype html>\\n'+clone.outerHTML],{type:'text/html;charset=utf-8'});const link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download='akt-po-domu.html';link.click();setTimeout(()=>URL.revokeObjectURL(link.href),1000)}</script></body></html>`;
    const win = window.open('', '_blank');
    if (!win) {
      showNotice('Браузер заблокировал открытие акта по дому. Разрешите всплывающие окна для сайта.');
      return;
    }
    win.document.open();
    win.document.write(html);
  ...
  function repairActKindLabel(kind) {
    if (kind === 'fact') return 'Фактические работы';
    if (kind === 'all') return 'Плановые и фактические работы';
    return 'Плановые работы';
  }

  function repairActRowInPeriod(date, fallbackYear, periodMode, year, month) {
    const rowYear = String(date ? date.getFullYear() : fallbackYear || '');
    const rowMonth = date ? String(date.getMonth() + 1).padStart(2, '0') : '';
    if (periodMode === 'year') return rowYear === year;
    if (periodMode === 'month') return rowYear === year && rowMonth === month;
    return true;
  }

  function repairPlanActRow(row) {
    const price = repairPlanActPrice(row);
    return {
      source: 'plan',
      address: row.address || row.house || '',
      house: row.house || row.address || '',
      ...
      actPeriod: 'План: ' + (row.deadlineRaw || (row.startDate ? formatDate(row.startDate) : '')),
      actDate: row.startDate || null,
      amount: row.amount || '',
      unit: row.unit || '',
      actPrice: price,
    };
  ...
  function repairFactActRow(row) {
    return {
      source: 'fact',
      address: row.address || row.house || '',
      house: row.house || row.address || '',
      workType: row.workType || '',
      deadlineRaw: row.actDate ? formatDate(row.actDate) : '',
      actPeriod: 'Факт: ' + (row.actDate ? formatDate(row.actDate) : row.status || 'выполнено'),
      actDate: row.actDate || null,
      amount: row.amount || '',
      unit: row.unit || '',
      actPrice: Number(row.cost || 0) || 0,
    };
  }

  function repairCompletedPlanFactActRow(row) {
    const price = repairActPrice(row);
    return {
      source: 'fact-plan-status',
      address: row.address || row.house || '',
      house: row.house || row.address || '',
      workType: row.workType || '',
      deadlineRaw: row.factDate ? formatDate(row.factDate) : row.deadlineRaw || '',
      actPeriod: 'Факт: ' + (row.factDate ? formatDate(row.factDate) : row.deadlineRaw || 'выполнено'),
      actDate: row.factDate || row.startDate || null,
      amount: row.amount || '',
      unit: row.unit || '',
      actPrice: price,
    };
  ...
  function dedupeRepairActRows(rows) {
    const seen = new Set();
    return rows.filter((row) => {
      const key = [
        normalizeRepairAddress(row.house || row.address),
        normalizeRepairWork(row.workType),
        row.actDate ? row.actDate.toISOString().slice(0, 10) : '',
        Math.round(Number(row.actPrice || 0)),
      ].join('|');
  ...
  function repairPlanActPrice(row) {
    const approved = state.approvedEstimates[row.id] || state.approvedEstimates[String(row.id)] || null;
    if (approved && approved.totals) {
      const total = Number(approved.totals.total);
      if (Number.isFinite(total) && total > 0) return total;
    }
    return Number(row.plannedCost || row.housePlanTotal || 0) || 0;
  }

  function repairActPeriodLabel(rows, periodMode) {
    const year = els.repairYearFilter ? els.repairYearFilter.value : '';
    const month = els.repairMonthFilter ? els.repairMonthFilter.value : '';
    if (periodMode === 'year' && year) return year + ' год';
    if (month && year) return monthNameByNumber(month) + ' ' + year;
    if (month) return monthNameByNumber(month);
    const dates = rows.map((row) => row.actDate || row.factDate || row.startDate).filter(Boolean).sort((a, b) => a - b);
    if (dates.length) return formatShortMonth(dates[0]) + ' ' + dates[0].getFullYear();
  ...
  function detectRepairComplex(address) {
    ...
    const known = knownRepairComplexByAddress(raw);
  ...
  function knownRepairComplexByAddress(address) {
    const key = normalizeRepairAddress(address);
    if (!key) return '';
    const aliases = [
      { complex: 'Зарядное', patterns: [/4(й|и|ой)?пятилетки/, /четверт.*пятилет/] },
      { complex: 'Точки', patterns: [/калинин/] },
    ];
    const hit = aliases.find((item) => item.patterns.some((pattern) => pattern.test(key)));
    return hit ? hit.complex : '';
  }

  function buildRepairComplexMap() {
    const map = new Map();
    ...
    state.ossRows.forEach((row) => remember(row.object, row.complex));
    state.repairRows.forEach((row) => remember(row.house || row.address, row.complex));
    state.repairFactRows.forEach((row) => remember(row.house || row.address, row.complex));
    return map;
  }

  function resolveRepairComplexFromMap(address, complexMap) {
    const known = knownRepairComplexByAddress(address);
    if (known) return known;
    const key = normalizeRepairAddress(address);
    if (!key) return '';
    if (complexMap.has(key)) return complexMap.get(key);
    for (const [mapKey, complex] of complexMap.entries()) {
      if (key.includes(mapKey) || mapKey.includes(key)) return complex;
    }
    return '';
  }

  function reconcileRepairComplexes() {
    const complexMap = buildRepairComplexMap();
    const reconcile = (row) => {
      const address = row.house || row.address || row.object || '';
      const complex = resolveRepairComplexFromMap(address, complexMap);
      if (!complex) return;
      row.complex = complex;
      row.house = stripRepairComplex(row.address || row.house || address, complex);
      row.houseKey = normalizeRepairAddress(row.house || row.address || address);
    };
    state.repairRows.forEach(reconcile);
    state.repairFactRows.forEach(reconcile);
  }
  ...
  function repairOssQuorumTarget(row) {
    const text = [row.workType, row.address].join(' ').toLowerCase();
    return /(шлагбаум|общее имущество|общего имущества|кондиционер|фасад|реклам|пользован)/.test(text) ? 67 : 51;
  }

  function repairEstimateTotal(row) {
    const approved = state.approvedEstimates[row.id] || state.approvedEstimates[String(row.id)];
    return Number(approved && approved.totals && approved.totals.total) || Number(row.factCost || 0) || row.plannedCost || row.housePlanTotal || 0;
  }

  function repairActPrice(row, estimatePayload) {
    const estimate = estimatePayload || state.approvedEstimates[row.id] || state.approvedEstimates[String(row.id)] || null;
    if (estimate && estimate.totals) {
      const total = Number(estimate.totals.total);
      if (Number.isFinite(total) && total > 0) return total;
    }
    const fact = Number(row.factCost || 0);
    if (Number.isFinite(fact) && fact > 0) return fact;
    return Number(row.plannedCost || row.housePlanTotal || 0) || 0;
  }

  function repairPaymentRows(row) {
    const total = repairEstimateTotal(row);
    const totalArea = Math.max(1, Number(row.houseArea) || 0);
    return [40, 60, 80, 100].map((area) => {
      const oneTime = totalArea ? total / totalArea * area : 0;
      return { area, oneTime, month3: oneTime / 3, month6: oneTime / 6 };
    });
  }

  function repairOssQuestions(row) {
    const total = repairEstimateTotal(row);
    return [
      'Утвердить выполнение работ: ' + (row.workType || 'текущий ремонт') + ' по адресу ' + (row.address || row.house || '') + '.',
      'Утвердить сметную стоимость работ в размере ' + (formatMoney(total) || 0) + ' руб.',
      'Утвердить порядок финансирования работ собственниками помещений пропорционально площади принадлежащих помещений.',
      'Уполномочить управляющую организацию организовать выполнение работ, контроль качества и оформление документов.',
    ];
  }
  ...
  function loadQuestionLibrary() {
    try {
      const raw = localStorage.getItem('oss-question-library');
  ...
  function saveQuestionLibrary() {
    try {
      localStorage.setItem('oss-question-library', JSON.stringify(state.questionLibrary || defaultQuestionLibrary));
    } catch (error) {
      console.warn('Не удалось сохранить библиотеку вопросов ОСС', error);
    }
  }

  async function loadQuestionLibraryFromGoogle(force) {
    try {
      const url = force ? questionLibraryApiUrl + '?refresh=' + Date.now() : questionLibraryApiUrl;
      const response = await fetch(url, { cache: force ? 'no-store' : 'default' });
      if (!response.ok) throw new Error('HTTP ' + response.status);
      const payload = await response.json();
      const categories = normalizeQuestionLibrary(payload.categories || []);
      if (!categories.length) throw new Error('пустая библиотека');
      state.questionLibrary = categories;
      state.questionLibrarySource = payload.source || 'google-sheets';
      state.questionLibraryCount = payload.count || categories.reduce((sum, group) => sum + (group.questions || []).length, 0);
      saveQuestionLibrary();
      renderQuestionBuilder();
    } catch (error) {
      state.questionLibrarySource = 'local-fallback';
      console.warn('Не удалось загрузить Google-библиотеку вопросов', error);
      renderQuestionBuilder();
    }
  }

  function normalizeQuestionLibrary(categories) {
    return (categories || []).map((group) => {
      const category = String(group.category || group.sheetTitle || 'Библиотека').trim();
      const questions = (group.questions || []).map((item) => {
        if (typeof item === 'string') {
          return { question: item, quorum: detectQuestionQuorum(item), keywords: [], type: category, category };
        }
        const question = String(item.question || item.text || item.title || '').trim();
        if (!question) return null;
        return {
          ...item,
          question,
          category: item.category || category,
          quorum: normalizeQuestionQuorum(item.quorum || item.quorumRaw || detectQuestionQuorum(question)),
          keywords: Array.isArray(item.keywords) ? item.keywords : String(item.keywords || '').split(/[;,|]/).map((word) => word.trim()).filter(Boolean),
          type: item.type || category,
        };
      }).filter(Boolean);
      return { category, questions };
    }).filter((group) => group.category && group.questions.length);
  }

  function normalizeQuestionQuorum(value) {
    const text = String(value || '').toLowerCase();
    if (/(67|2\/3|две трети)/.test(text)) return 67;
    const number = toNumber(text);
    if (number >= 60) return 67;
    return 51;
  }

  function loadQuestionSelections() {
    try {
      const raw = localStorage.getItem('oss-question-selections');
      const parsed = raw ? JSON.parse(raw) : {};
      return parsed && typeof parsed === 'object' ? parsed : {};
    } catch (error) {
      return {};
  ...
  function saveQuestionSelections() {
    try {
      localStorage.setItem('oss-question-selections', JSON.stringify(state.questionSelections || {}));
    } catch (error) {
      console.warn('Не удалось сохранить вопросы ОСС', error);
    }
  }

  function documentQuestionsKey(row = selectedDocumentRepairRow()) {
    const scope = els.documentScopeSelect ? els.documentScopeSelect.value : 'single';
    const base = row ? [scope, row.complex, row.house || row.address, scope === 'single' ? row.workType : 'house'].filter(Boolean).join('|') : scope + '|default';
    return normalizeText(base || 'default') || 'default';
  }

  function applyQuestionTemplate(question, row) {
    const rawQuestion = typeof question === 'string' ? question : (question && (question.question || question.text || question.title)) || '';
    if (!row) return question;
    const scope = els.documentScopeSelect ? els.documentScopeSelect.value : 'single';
    const houseRows = scope === 'house' ? documentHouseRows(row) : [row];
    const total = scope === 'house'
      ? houseRows.reduce((sum, item) => sum + repairEstimateTotal(item), 0)
      : repairEstimateTotal(row);
    const works = houseRows.map((item) => item.workType).filter(Boolean).join('; ');
    return String(rawQuestion || '')
      .replace(/\{address\}/g, row.address || row.house || '')
      .replace(/\{house\}/g, row.house || row.address || '')
      .replace(/\{workType\}/g, row.workType || 'текущий ремонт')
      .replace(/\{works\}/g, works || row.workType || 'текущий ремонт')
      .replace(/\{total\}/g, formatMoney(total) || '0');
  }

  function detectQuestionQuorum(question) {
    return detectRequiredQuorum(question) >= 0.67 ? 67 : 51;
  }

  function questionText(question) {
    return typeof question === 'string' ? question : String((question && (question.question || question.text || question.title)) || '');
  }

  function questionQuorum(question, renderedText = '') {
    if (question && typeof question === 'object' && question.quorum) return normalizeQuestionQuorum(question.quorum);
    return detectQuestionQuorum(renderedText || questionText(question));
  }

  function allLibraryQuestions(library = state.questionLibrary) {
    return (Array.isArray(library) ? library : defaultQuestionLibrary).flatMap((group) => {
      return (group.questions || []).map((question) => ({
        ...((typeof question === 'object' && question) || {}),
        category: (question && question.category) || group.category,
        question: questionText(question),
      }));
    }).filter((item) => item.question);
  }

  function questionMatchScore(question, row) {
    if (!row) return 0;
    const target = normalizeText([row.workType, row.house, row.address, row.complex, row.region].filter(Boolean).join(' '));
    ...
    const questionTextNormalized = normalizeText(questionSource);
    let score = 0;
    normalizeText(row.workType || '').split(' ').filter((word) => word.length > 3).forEach((word) => {
      if (questionTextNormalized.includes(word)) score += 8;
    });
    normalizeText(row.complex || '').split(' ').filter((word) => word.length > 3).forEach((word) => {
      if (questionTextNormalized.includes(word)) score += 2;
    });
    if (/текущ|ремонт|смет|стоим|финанс|работ/.test(questionTextNormalized) && /ремонт|работ/.test(target)) score += 10;
    if (/общ.*имуществ|шлагбаум|кондиционер|фасад/.test(questionTextNormalized) && /шлагбаум|кондиционер|фасад|общ.*имуществ/.test(target)) score += 12;
    if (/администратор|гис|прием решений|сообщени/.test(questionTextNormalized)) score += 3;
    return score;
  }

  function recommendedLibraryQuestions(row) {
    const items = allLibraryQuestions();
    const scored = items.map((question) => ({ question, score: questionMatchScore(question, row) }))
      .filter((item) => item.score > 0)
      .sort((a, b) => b.score - a.score)
      .slice(0, 12)
      .map((item) => ({ ...item.question, recommendedScore: item.score }));
    return scored.length ? scored : items.slice(0, 8);
  }

  function selectedQuestionsRequiredQuorum(questions, row = selectedDocumentRepairRow()) {
    const max = (questions || []).reduce((current, question) => {
      const meta = findQuestionMeta(question, row);
      return Math.max(current, questionQuorum(meta, question));
    }, 51);
    return max;
  }

  function findQuestionMeta(renderedText, row) {
    const normalizedRendered = normalizeText(renderedText);
    return allLibraryQuestions().find((question) => normalizeText(applyQuestionTemplate(question, row)) === normalizedRendered) || null;
  }

  function defaultDocumentQuestions(row) {
    if (!row) return [];
    const recommended = recommendedLibraryQuestions(row).slice(0, 4).map((question) => applyQuestionTemplate(question, row)).filter(Boolean);
    if (recommended.length >= 2) return Array.from(new Set(recommended));
    const scope = els.documentScopeSelect ? els.documentScopeSelect.value : 'single';
    if (scope !== 'house') return repairOssQuestions(row);
    const houseRows = documentHouseRows(row);
    const total = houseRows.reduce((sum, item) => sum + repairEstimateTotal(item), 0);
    const works = houseRows.map((item) => item.workType).filter(Boolean);
    return [
      'Утвердить выполнение комплекса работ по текущему ремонту по адресу ' + (row.address || row.house || '') + ': ' + works.join('; ') + '.',
      'Утвердить предельную сметную стоимость комплекса работ в размере ' + (formatMoney(total) || 0) + ' руб.',
      'Утвердить порядок финансирования работ клиентами пропорционально площади принадлежащих помещений.',
      'Уполномочить управляющую организацию организовать выполнение работ, контроль качества и оформление документов.',
    ];
  }

  function documentHouseRows(row) {
    if (!row) return [];
    const houseKey = row.houseKey || normalizeRepairAddress(row.house || row.address);
    return state.repairRows.filter((item) => (item.houseKey || normalizeRepairAddress(item.house || item.address)) === houseKey);
  }

  function currentDocumentQuestions(row = selectedDocumentRepairRow()) {
    if (!row) return [];
    const key = documentQuestionsKey(row);
    const saved = state.questionSelections && state.questionSelections[key];
    if (Array.isArray(saved) && saved.length) return saved;
    return defaultDocumentQuestions(row);
  }

  function setCurrentDocumentQuestions(questions, row = selectedDocumentRepairRow()) {
    if (!row) return;
    const key = documentQuestionsKey(row);
    if (!state.questionSelections || typeof state.questionSelections !== 'object') state.questionSelections = {};
    state.questionSelections[key] = (questions || []).map((item) => String(item || '').trim()).filter(Boolean);
    saveQuestionSelections();
    renderQuestionBuilder();
  }

  function renderQuestionBuilder() {
    if (!els.questionCategorySelect || !els.questionLibraryList || !els.selectedQuestionsList) return;
    const row = selectedDocumentRepairRow();
    const library = normalizeQuestionLibrary(Array.isArray(state.questionLibrary) && state.questionLibrary.length ? state.questionLibrary : defaultQuestionLibrary);
    const categories = ['Рекомендовано', ...library.map((item) => item.category).filter(Boolean)];
    const previousCategory = els.questionCategorySelect.value;
    els.questionCategorySelect.innerHTML = categories.map((category) => '<option value="' + escapeAttr(category) + '">' + escapeHtml(category) + '</option>').join('');
    if (previousCategory && categories.includes(previousCategory)) els.questionCategorySelect.value = previousCategory;
    const category = els.questionCategorySelect.value || categories[0] || '';
    const selected = currentDocumentQuestions(row);
    const groupQuestions = category === 'Рекомендовано'
      ? recommendedLibraryQuestions(row)
      : ((library.find((item) => item.category === category) || library[0] || { questions: [] }).questions || []);
    els.questionLibraryList.innerHTML = groupQuestions.map((question, index) => {
      const text = applyQuestionTemplate(question, row);
      const used = selected.includes(text);
      const quorum = questionQuorum(question, text);
      const source = [question.category, question.type, question.sheetTitle].filter(Boolean).filter((item, itemIndex, list) => list.indexOf(item) === itemIndex).join(' · ');
      return '<article class="question-card">'
        + '<strong>Вопрос из библиотеки</strong>'
        + '<div class="question-meta"><span>кворум ' + quorum + '%</span>' + (source ? '<span>' + escapeHtml(source) + '</span>' : '') + '</div>'
        + '<small>' + escapeHtml(text) + '</small>'
        + '<div class="question-card-actions"><button type="button" data-add-library-question="' + index + '"' + (used ? ' disabled' : '') + '>' + (used ? 'Добавлен' : 'Добавить') + '</button></div>'
        + '</article>';
    }).join('') || '<div class="muted">В этой категории пока нет вопросов.</div>';
    els.selectedQuestionsList.innerHTML = selected.map((question, index) => {
      return '<article class="question-card">'
        + '<strong>Вопрос №' + (index + 1) + '</strong>'
        + '<textarea data-question-index="' + index + '">' + escapeHtml(question) + '</textarea>'
        + '<div class="question-card-actions">'
        + '<button type="button" data-question-up="' + index + '"' + (index === 0 ? ' disabled' : '') + '>Выше</button>'
        + '<button type="button" data-question-down="' + index + '"' + (index === selected.length - 1 ? ' disabled' : '') + '>Ниже</button>'
        + '<button class="danger" type="button" data-question-remove="' + index + '">Удалить</button>'
        + '</div></article>';
    }).join('') || '<div class="muted">Добавьте вопросы из библиотеки или вручную.</div>';
    if (els.selectedQuestionsCount) els.selectedQuestionsCount.textContent = selected.length + ' ' + pluralRu(selected.length, 'вопрос', 'вопроса', 'вопросов') + ' · кворум ' + selectedQuestionsRequiredQuorum(selected, row) + '%';
  }

  function handleQuestionLibraryClick(event) {
    const button = event.target.closest('[data-add-library-question]');
    if (!button) return;
    const row = selectedDocumentRepairRow();
    const library = normalizeQuestionLibrary(Array.isArray(state.questionLibrary) && state.questionLibrary.length ? state.questionLibrary : defaultQuestionLibrary);
    const category = els.questionCategorySelect.value || 'Рекомендовано';
    const sourceQuestions = category === 'Рекомендовано'
      ? recommendedLibraryQuestions(row)
      : ((library.find((item) => item.category === category) || library[0] || { questions: [] }).questions || []);
    const question = sourceQuestions[Number(button.dataset.addLibraryQuestion)] || '';
    const selected = currentDocumentQuestions(row);
    const text = applyQuestionTemplate(question, row);
    if (text && !selected.includes(text)) setCurrentDocumentQuestions([...selected, text], row);
  }

  function addCustomDocumentQuestion() {
    const text = els.customQuestionInput ? els.customQuestionInput.value.trim() : '';
    const row = selectedDocumentRepairRow();
    if (!text) { showNotice('Введите текст вопроса для повестки.'); return; }
    setCurrentDocumentQuestions([...currentDocumentQuestions(row), text], row);
    els.customQuestionInput.value = '';
  }

  function handleSelectedQuestionInput(event) {
    const input = event.target.closest('[data-question-index]');
    if (!input) return;
    const row = selectedDocumentRepairRow();
    const questions = currentDocumentQuestions(row);
    questions[Number(input.dataset.questionIndex)] = input.value;
    const key = documentQuestionsKey(row);
    if (!state.questionSelections || typeof state.questionSelections !== 'object') state.questionSelections = {};
    state.questionSelections[key] = questions;
    saveQuestionSelections();
  }

  function handleSelectedQuestionClick(event) {
    const row = selectedDocumentRepairRow();
    const questions = currentDocumentQuestions(row);
    ...
    if (remove) questions.splice(Number(remove.dataset.questionRemove), 1);
      ...
      const index = Number(up.dataset.questionUp);
      if (index > 0) [questions[index - 1], questions[index]] = [questions[index], questions[index - 1]];
    } else if (down) {
      const index = Number(down.dataset.questionDown);
      if (index < questions.length - 1) [questions[index + 1], questions[index]] = [questions[index], questions[index + 1]];
    } else return;
    setCurrentDocumentQuestions(questions, row);
  }

  function saveCurrentDocumentQuestions() {
    const row = selectedDocumentRepairRow();
    const questions = Array.from(els.selectedQuestionsList.querySelectorAll('[data-question-index]')).map((input) => input.value.trim()).filter(Boolean);
    setCurrentDocumentQuestions(questions, row);
    showNotice('Повестка сохранена для выбранного объекта.');
  }

  function resetCurrentDocumentQuestions() {
    const row = selectedDocumentRepairRow();
    if (!row || !state.questionSelections) return;
    delete state.questionSelections[documentQuestionsKey(row)];
    saveQuestionSelections();
    renderQuestionBuilder();
    showNotice('Вернул типовую повестку для выбранного объекта.');
  }



  function loadOssTemplates() {
    try {
      const raw = localStorage.getItem('oss-document-templates');
      const parsed = raw ? JSON.parse(raw) : {};
      return parsed && typeof parsed === 'object' ? parsed : {};
    } catch (error) {
      return {};
  ...
  function saveOssTemplates() {
    try {
      localStorage.setItem('oss-document-templates', JSON.stringify(state.ossTemplates || {}));
    } catch (error) {
      console.warn('Не удалось сохранить шаблоны ОСС', error);
    }
  }

  function documentTemplateKey(row = selectedDocumentRepairRow()) {
    const base = row ? [row.complex, row.house || row.address, row.address || row.house].filter(Boolean).join('|') : 'default';
    return normalizeText(base || 'default') || 'default';
  }

  function currentOssTemplateSet(create = false) {
    if (!state.ossTemplates || typeof state.ossTemplates !== 'object') state.ossTemplates = {};
    if (!state.ossTemplates.houses || typeof state.ossTemplates.houses !== 'object') {
      if (!create) return {};
      state.ossTemplates.houses = {};
    }
    const key = documentTemplateKey();
    if (!state.ossTemplates.houses[key] && create) state.ossTemplates.houses[key] = {};
    return state.ossTemplates.houses[key] || {};
  }

  function templateForKind(kind) {
    if (!kind || !state.ossTemplates) return '';
    const houseTemplates = currentOssTemplateSet(false);
    if (houseTemplates[kind] && houseTemplates[kind].base64) return houseTemplates[kind].base64;
    return state.ossTemplates[kind] && state.ossTemplates[kind].base64 ? state.ossTemplates[kind].base64 : '';
  }

  async function loadOssTemplateFromFile(event, kind) {
    const file = event.target.files && event.target.files[0];
    if (!file) return;
    if (!/\.docx$/i.test(file.name)) {
      showNotice('Шаблон ' + (kind === 'ballot' ? 'бюллетеня' : 'сообщения') + ' должен быть в формате DOCX.');
      event.target.value = '';
      return;
    }
    const base64 = await fileToBase64(file);
    const houseTemplates = currentOssTemplateSet(true);
    const row = selectedDocumentRepairRow();
    houseTemplates[kind] = {
      name: file.name,
      base64,
      savedAt: new Date().toISOString(),
      houseKey: documentTemplateKey(row),
      houseName: row ? (row.house || row.address || '') : '',
    };
    saveOssTemplates();
    renderTemplateStatus();
    markOssDocButtonReady(event.target, kind === 'ballot' ? '✓ Шаблон бюллетеня' : '✓ Шаблон сообщения');
    showNotice('Шаблон загружен и сохранен для выбранного дома: ' + file.name + '.');
  }

  function markOssDocButtonReady(input, text, ready = true) {
    const label = input && input.closest ? input.closest('.file-btn') : null;
    if (!label) return;
    if (ready) label.classList.add('is-ready');
    else label.classList.remove('is-ready');
  ...
  function renderTemplateStatus() {
    ...
    const houseTemplates = currentOssTemplateSet(false);
    const legacyBallot = state.ossTemplates && state.ossTemplates.ballot;
    const legacyNotice = state.ossTemplates && state.ossTemplates.notice;
    const ballotTemplate = houseTemplates.ballot || legacyBallot;
    const noticeTemplate = houseTemplates.notice || legacyNotice;
    const ballot = ballotTemplate ? ballotTemplate.name : 'базовый шаблон';
    const notice = noticeTemplate ? noticeTemplate.name : 'базовый шаблон';
    if (els.ballotTemplateInput) markOssDocButtonReady(els.ballotTemplateInput, ballotTemplate ? '✓ Шаблон бюллетеня' : 'Шаблон бюллетеня DOCX', !!ballotTemplate);
    if (els.noticeTemplateInput) markOssDocButtonReady(els.noticeTemplateInput, noticeTemplate ? '✓ Шаблон сообщения' : 'Шаблон сообщения DOCX', !!noticeTemplate);
    const row = selectedDocumentRepairRow();
    const house = row && (row.house || row.address) ? ' · дом: ' + (row.house || row.address) : '';
    els.templateStatus.textContent = 'Бюллетень: ' + ballot + ' · Сообщение: ' + notice + house;
  }

  function documentRowsBase() {
    return (state.repairFiltered.length ? state.repairFiltered : state.repairRows).slice(0, 1000);
  }

  function documentRowComplex(row) {
    return row.complex || row.region || 'Без ЖК';
  }

  function documentRowHouse(row) {
    return row.house || row.address || 'Без адреса';
  }
  ...
  function selectedDocumentRepairRow() {
    const id = Number(els.documentRepairSelect && els.documentRepairSelect.value);
    return state.repairRows.find((row) => row.id === id) || documentRowsBase()[0] || state.repairRows[0] || null;
  }

  function handleDocumentComplexChange() {
    if (els.documentHouseSelect) els.documentHouseSelect.value = '';
    renderOssDocumentsModule();
  }

  function handleDocumentHouseChange() {
    renderOssDocumentsModule();
  }

  function renderOssDocumentsModule() {
    if (!els.documentRepairSelect) return;
    const previousComplex = els.documentComplexSelect ? els.documentComplexSelect.value : '';
    const previousHouse = els.documentHouseSelect ? els.documentHouseSelect.value : '';
    const previousRepair = els.documentRepairSelect.value;
    const rows = documentRowsBase();
    const complexes = uniqSorted(rows.map(documentRowComplex));
    const selectedComplex = setSelectOptions(els.documentComplexSelect, complexes, previousComplex, 'Нет ЖК');
    const houseRows = selectedComplex ? rows.filter((row) => documentRowComplex(row) === selectedComplex) : rows;
    const houses = uniqSorted(houseRows.map(documentRowHouse));
    const selectedHouse = setSelectOptions(els.documentHouseSelect, houses, previousHouse, 'Нет адреса');
    const workRows = houseRows.filter((row) => !selectedHouse || documentRowHouse(row) === selectedHouse);
    els.documentRepairSelect.innerHTML = workRows.map((row) => {
      const label = row.workType || [documentRowComplex(row), documentRowHouse(row)].filter(Boolean).join(' · ');
      return '<option value="' + row.id + '">' + escapeHtml(label) + '</option>';
    }).join('') || '<option value="">Нет работ</option>';
    if (previousRepair && Array.from(els.documentRepairSelect.options).some((option) => option.value === previousRepair)) {
      els.documentRepairSelect.value = previousRepair;
    }
    renderTemplateStatus();
    renderQuestionBuilder();
    renderOwnersRegistry();
  }

  async function loadOwnersFromFile(event) {
    const file = event.target.files && event.target.files[0];
    if (!file) return;
    if (!/\.csv$/i.test(file.name)) {
      state.ownersRows = [];
      state.ownersErrors = ['Файл Excel нужно сохранить как CSV: Файл → Скачать → CSV. Так персональные данные остаются в браузере.'];
      renderOwnersRegistry();
      return;
    }
    const text = await readOwnersCsvText(file);
    const matrix = parseOwnersCsv(text).filter((row) => row.some((cell) => String(cell || '').trim()));
    if (!matrix.length) {
      state.ownersRows = [];
      state.ownersErrors = ['CSV пустой или не читается.'];
      renderOwnersRegistry();
      return;
    }
    const headerIndex = findOwnersHeaderIndex(matrix);
    if (headerIndex < 0) {
      state.ownersRows = [];
      state.ownersErrors = ['Не нашел строку заголовков. Нужны колонки: номер помещения, площадь, ФИО или имя/фамилия/отчество.'];
      renderOwnersRegistry();
      return;
    }
    const headers = matrix[headerIndex].map(cleanHeader);
    const errors = [];
    const rows = matrix.slice(headerIndex + 1)
      .map((row, index) => normalizeOwnerRow(headers, row, headerIndex + index + 2))
      .filter((owner) => owner.hasAnyData);
    rows.forEach((owner) => {
      if (!owner.flat) owner.errors.push('нет помещения');
      if (!owner.name) owner.errors.push('нет ФИО/клиента');
      if (!owner.area) owner.errors.push('нет площади');
      if (owner.errors.length) errors.push('Строка ' + owner.sourceRow + ': ' + owner.errors.join(', '));
    });
    state.ownersRows = rows;
    state.ownersErrors = errors;
    renderOwnersRegistry();
    markOssDocButtonReady(els.ownersFileInput, '✓ Реестр загружен');
    showNotice('Реестр клиентов загружен: ' + rows.length + ' строк. Кодировка и разделитель распознаны автоматически.');
  }

  async function readOwnersCsvText(file) {
    const buffer = await file.arrayBuffer();
    const variants = ['utf-8', 'windows-1251', 'ibm866'].map((encoding) => {
      try {
        const text = new TextDecoder(encoding).decode(buffer);
        return { encoding, text, score: scoreOwnersCsvText(text) };
      } catch (error) {
  ...
  function scoreOwnersCsvText(text) {
    const normalized = normalizeText(text);
    let score = 0;
    ['номер помещения', 'площадь помещения', 'имя собственник', 'фамилия собственник', 'снилс', 'выписка'].forEach((needle) => {
      if (normalized.includes(needle)) score += 20;
    });
    score += (text.match(/[А-Яа-яЁё]/g) || []).length / 100;
    score -= (text.match(/�/g) || []).length * 10;
    score -= (normalized.match(/[џђѓєѕїјљњћќў]/g) || []).length * 4;
    return score;
  }
  ...
  function parseOwnersCsv(text) {
    ...
    const delimiter = candidates
      .map((item) => ({ item, count: (firstLines.match(new RegExp(item === '\t' ? '\t' : escapeRegExp(item), 'g')) || []).length }))
  ...
  function findOwnersHeaderIndex(matrix) {
    let best = { index: -1, score: 0 };
    matrix.slice(0, 25).forEach((row, index) => {
      const text = normalizeText(row.join(' '));
      let score = 0;
      if (text.includes('номер помещения') || text.includes('помещение')) score += 3;
      if (text.includes('площадь')) score += 3;
      if (text.includes('собственник') || text.includes('клиент')) score += 3;
      if (text.includes('снилс') || text.includes('огрн')) score += 1;
      if (score > best.score) best = { index, score };
    });
    return best.score >= 6 ? best.index : -1;
  }

  function normalizeOwnerRow(headers, row, sourceRow) {
    const flat = getByHeader(headers, row, [/номер.*помещ/, /^помещ/, /кварт/, /^№$/i]);
    const areaRaw = getByHeader(headers, row, [/площад.*помещ/, /количество.*голос/, /площад/]);
    const name = ownerFullName(headers, row);
    const share = getByHeader(headers, row, [/дол/]);
    const docType = getByHeader(headers, row, [/тип.*документ/]);
    const docNumber = getByHeader(headers, row, [/номер.*регистрац/, /регистрац/]);
    const docDate = getByHeader(headers, row, [/дата.*регистрац/]);
    const document = [docType, docNumber, docDate].filter(Boolean).join(' от ') || getByHeader(headers, row, [/документ/, /основан/, /право/, /егрн/, /свидетель/]);
    const snils = getByHeader(headers, row, [/снилс/, /огрн/, /инн/]);
    const hasAnyData = row.some((cell) => String(cell || '').trim());
    const premiseArea = toNumber(areaRaw);
    const voteArea = premiseArea * ownerShareRatio(share);
    return { sourceRow, flat, name, areaRaw, premiseArea, area: voteArea || premiseArea, share, document, snils, hasAnyData, errors: [] };
  }

  function ownerShareRatio(value) {
  ...
  function ownerFullName(headers, row) {
    const direct = getByHeader(headers, row, [/фио/, /клиент/, /правооблад/, /название.*организац/, /организац/]);
    if (direct) return direct;
    const last = getByHeader(headers, row, [/фамилия.*собствен/, /^фамилия$/]);
    const first = getByHeader(headers, row, [/имя.*собствен/, /^имя$/]);
    const middle = getByHeader(headers, row, [/отчество.*собствен/, /^отчество$/]);
    return [last, first, middle].filter(Boolean).join(' ').replace(/\s*,\s*/g, ', ');
  }

  function renderOwnersRegistry() {
    if (!els.ownersPreviewBody) return;
    const rows = state.ownersRows || [];
    const validRows = rows.filter((row) => !row.errors.length);
    const areaTotal = validRows.reduce((sum, row) => sum + (Number(row.area) || 0), 0);
    els.ownersRowsCount.textContent = formatNumber(rows.length);
    els.ownersValidCount.textContent = formatNumber(validRows.length);
    els.ownersErrorCount.textContent = formatNumber((state.ownersErrors || []).length);
    els.ownersAreaTotal.textContent = formatNumber(areaTotal);
    els.ownersPreviewSummary.textContent = rows.length ? validRows.length + ' готово из ' + rows.length : 'загрузите CSV';
    els.ownersPreviewBody.innerHTML = rows.slice(0, 120).map((row) => {
      const status = row.errors.length ? '<span class="risk-text">' + escapeHtml(row.errors.join(', ')) + '</span>' : '<span class="ok-text">готово</span>';
      const areaText = row.premiseArea && row.share && row.share !== '1' ? formatNumber(row.area) + ' из ' + formatNumber(row.premiseArea) : (row.area ? formatNumber(row.area) : '');
      return '<tr><td>' + escapeHtml(row.flat) + '</td><td>' + escapeHtml(row.name) + '</td><td>' + areaText + '</td><td>' + escapeHtml(row.share) + '</td><td>' + escapeHtml(row.document) + '</td><td>' + status + '</td></tr>';
    }).join('') || '<tr><td colspan="6" class="muted">Загрузите CSV-реестр клиентов для массового формирования бюллетеней.</td></tr>';
    const checklist = [];
    const selected = selectedDocumentRepairRow();
    if (!selected) checklist.push('Выберите работу или дом для запуска ОСС.');
    if (selected && !state.approvedEstimates[selected.id] && !state.approvedEstimates[String(selected.id)]) checklist.push('По выбранной работе нет согласованной инженером сметы.');
    if (!rows.length) checklist.push('Загрузите реестр клиентов с помещением, ФИО и площадью.');
    if (state.ownersErrors && state.ownersErrors.length) checklist.push(...state.ownersErrors.slice(0, 12));
    if (state.ownersErrors && state.ownersErrors.length > 12) checklist.push('Еще ошибок: ' + (state.ownersErrors.length - 12));
    els.ownersErrorsList.innerHTML = checklist.map((item) => '<div class="owner-check-item">' + escapeHtml(item) + '</div>').join('') || '<div class="owner-check-item ok">Реестр выглядит готовым для формирования бюллетеней.</div>';
  }

  function clearOwnersRegistry() {
    state.ownersRows = [];
    state.ownersErrors = [];
    if (els.ownersFileInput) {
      els.ownersFileInput.value = '';
      markOssDocButtonReady(els.ownersFileInput, 'Загрузить реестр клиентов CSV', false);
    }
    renderOwnersRegistry();
  }

  function documentOssQuestions(row) {
    return currentDocumentQuestions(row);
  }

  function downloadSelectedOssNotice() {
    const row = selectedDocumentRepairRow();
    if (!row) return;
    downloadRepairOssNotice(row.id);
  }

  function downloadBulkOssBallots() {
    const row = selectedDocumentRepairRow();
    if (!row) { showNotice('Выберите работу для формирования бюллетеней.'); return; }
    const owners = (state.ownersRows || []).filter((owner) => !owner.errors.length);
    if (!owners.length) { showNotice('Нет корректных строк реестра клиентов для бюллетеней.'); return; }
    const questions = documentOssQuestions(row);
    const body = owners.map((owner) => ownerBallotHtml(row, owner, questions)).join('<div class="page-break"></div>');
    const title = 'Бюллетени ОСС ' + (row.house || row.address || '');
    downloadWordDoc('bulletins_OSS_' + safeFileName(row.house || row.address) + '_' + owners.length + '_clients.doc', title, body);
  }

  function ownerBallotHtml(row, owner, questions) {
    const total = repairEstimateTotal(row);
    const quorum = repairOssQuorumTarget(row);
    const start = els.documentStartDate && els.documentStartDate.value ? formatDate(new Date(els.documentStartDate.value)) : '';
    const end = els.documentEndDate && els.documentEndDate.value ? formatDate(new Date(els.documentEndDate.value)) : '';
    return '<h1>РЕШЕНИЕ СОБСТВЕННИКА</h1>'
      + '<p>помещения в многоквартирном доме, расположенном по адресу: <strong>' + escapeHtml(row.address || row.house || '') + '</strong></p>'
      + '<p>по вопросам внеочередного общего собрания собственников, проводимого в форме заочного голосования' + (start || end ? ' с ' + escapeHtml(start) + ' по ' + escapeHtml(end) : '') + '.</p>'
      + '<table><tbody>'
      + '<tr><th>№ помещения</th><th>Количество голосов, кв.м.</th></tr>'
      + '<tr><td>' + escapeHtml(owner.flat) + '</td><td>' + (owner.area ? formatNumber(owner.area) : '') + '</td></tr>'
      + '<tr><th>ФИО клиента / наименование юридического лица</th><th>СНИЛС / ОГРН</th></tr>'
      + '<tr><td>' + escapeHtml(owner.name) + '</td><td>' + escapeHtml(owner.snils) + '</td></tr>'
      + '<tr><th colspan="2">Сведения о праве собственности</th></tr>'
      + '<tr><td colspan="2">' + escapeHtml(owner.document || owner.share || '') + '</td></tr>'
      + '</tbody></table>'
      + '<p><strong>Вид работ:</strong> ' + escapeHtml(row.workType || '') + '</p>'
      + '<p><strong>Сметная стоимость:</strong> ' + (formatMoney(total) || 0) + ' руб. Требуемый кворум: ' + quorum + '%.</p>'
      + '<h2>ВОПРОСЫ, ПОСТАВЛЕННЫЕ НА ГОЛОСОВАНИЕ</h2>'
      + '<table><thead><tr><th>№</th><th>Вопрос</th><th>За</th><th>Против</th><th>Воздержался</th></tr></thead><tbody>'
      + questions.map((item, index) => '<tr><td>' + (index + 1) + '</td><td>' + escapeHtml(item) + '</td><td></td><td></td><td></td></tr>').join('')
      + '</tbody></table>'
      + '<p>Дата заполнения: <span class="line"></span> Подпись: <span class="line"></span></p>';
  }


  function selectedDocumentPayload() {
    const row = selectedDocumentRepairRow();
    if (!row) return null;
    const questions = documentOssQuestions(row);
    return {
      ...
      house: row.house || row.address || '',
      workType: row.workType || '',
      startDate: els.documentStartDate ? els.documentStartDate.value : '',
      endDate: els.documentEndDate ? els.documentEndDate.value : '',
      total: repairEstimateTotal(row),
      quorum: selectedQuestionsRequiredQuorum(questions, row),
      scope: els.documentScopeSelect ? els.documentScopeSelect.value : 'single',
      questions,
    };
  }

  async function postOssDocument(payload) {
    const response = await fetch(ossDocumentsApiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    if (!response.ok) {
      let message = 'Не удалось сформировать DOCX';
      try { const data = await response.json(); message = data.error || message; } catch (error) {}
      throw new Error(message);
    }
    const blob = await response.blob();
    const disposition = response.headers.get('content-disposition') || '';
    const match = disposition.match(/filename=\"?([^\";]+)\"?/i);
    downloadBlob(match ? match[1] : 'oss-document.docx', blob);
  }

  function validOwnersForDocuments() {
    return (state.ownersRows || []).filter((owner) => !owner.errors.length);
  }

  function currentTemplateNames() {
    const houseTemplates = currentOssTemplateSet(false);
    const legacyBallot = state.ossTemplates && state.ossTemplates.ballot;
    const legacyNotice = state.ossTemplates && state.ossTemplates.notice;
    return {
      ballot: (houseTemplates.ballot || legacyBallot || {}).name || 'базовый шаблон',
      notice: (houseTemplates.notice || legacyNotice || {}).name || 'базовый шаблон',
    };
  }

  function pdfDocumentShell(title, body, options = {}) {
    const style = `@page{size:A4;margin:14mm 12mm}*{box-sizing:border-box}body{margin:0;background:#eef2f6;color:#000;font-family:"Times New Roman",Times,serif;font-size:12pt;line-height:1.25}.toolbar{position:sticky;top:0;z-index:10;display:flex;gap:8px;flex-wrap:wrap;background:#fff;border-bottom:1px solid #cbd5e1;padding:10px 14px;font-family:Arial,sans-serif}.toolbar button{border:1px solid #8ea4bc;border-radius:8px;background:#f8fafc;padding:9px 12px;font-weight:800;cursor:pointer}.sheet{width:190mm;min-height:267mm;margin:16px auto;background:#fff;padding:14mm 13mm;box-shadow:0 2px 14px rgb(15 23 42 / .14)}h1{font-size:15pt;text-align:center;margin:0 0 14px;text-transform:uppercase}h2{font-size:13pt;margin:14px 0 8px}.center{text-align:center}.muted{color:#64748b}.small{font-size:10pt}.indent{text-indent:10mm}.question{margin:7px 0;text-indent:10mm}.meta{display:grid;grid-template-columns:38mm 1fr;gap:4px 8px;margin:10px 0}.meta strong{font-weight:700}.line{display:inline-block;border-bottom:1px solid #000;min-width:42mm;min-height:14px}.doc-table{width:100%;border-collapse:collapse;margin:10px 0}.doc-table th,.doc-table td{border:1px solid #000;padding:5px 6px;vertical-align:top}.doc-table th{text-align:center;font-weight:700}.vote{width:18mm;text-align:center}.page-break{page-break-after:always}.footer-note{margin-top:12px;font-size:10pt}.template-note{font:11px Arial,sans-serif;color:#64748b;margin-bottom:8px}.sign-grid{display:grid;grid-template-columns:1fr 1fr;gap:16mm;margin-top:18px}.sign-line{height:20px;border-bottom:1px solid #000}.nowrap{white-space:nowrap}@media print{body{background:#fff}.toolbar{display:none}.sheet{margin:0 auto;box-shadow:none;page-break-after:always}.sheet:last-child{page-break-after:auto}}`;
    const html = '<!doctype html><html lang="ru"><head><meta charset="utf-8"><title>' + escapeHtml(title) + '</title><style>' + style + '</style></head><body>'
      + '<div class="toolbar"><button onclick="window.print()">Печать / сохранить PDF</button><button onclick="downloadHtml()">Скачать HTML</button><span class="muted">Для PDF выберите печать и принтер “Сохранить как PDF”.</span></div>'
      + body
      + '<script>function downloadHtml(){const clone=document.documentElement.cloneNode(true);clone.querySelectorAll(".toolbar,script").forEach((el)=>el.remove());const blob=new Blob(["<!doctype html>\\n"+clone.outerHTML],{type:"text/html;charset=utf-8"});const link=document.createElement("a");link.href=URL.createObjectURL(blob);link.download=' + JSON.stringify((options.fileName || safeFileName(title)) + '.html') + ';link.click();setTimeout(()=>URL.revokeObjectURL(link.href),1000)}</script>'
      + '</body></html>';
    const blob = new Blob([html], { type: 'text/html;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const win = window.open(url, '_blank');
    if (!win) showNotice('Браузер заблокировал окно PDF. Разрешите всплывающие окна для дашборда.');
    setTimeout(() => URL.revokeObjectURL(url), 30000);
  }

  function ossQuestionsHtml(questions) {
    return (questions || []).map((question, index) => '<p class="question"><strong>Вопрос №' + (index + 1) + ':</strong> ' + escapeHtml(question) + '</p>').join('');
  }

  function openOssNoticePdf() {
    const payload = selectedDocumentPayload();
    if (!payload) { showNotice('Выберите работу для сообщения ОСС.'); return; }
    const names = currentTemplateNames();
    const questions = payload.questions || [];
    const start = payload.startDate ? formatDate(new Date(payload.startDate)) : '';
    const end = payload.endDate ? formatDate(new Date(payload.endDate)) : '';
    const body = '<main class="sheet">'
      + '<div class="template-note">Шаблон сообщения: ' + escapeHtml(names.notice) + '</div>'
      + '<h1>Сообщение о проведении ОСС</h1>'
      + '<p class="center"><strong>о проведении заочного внеочередного общего собрания собственников помещений</strong></p>'
      + '<p class="center">в многоквартирном доме, расположенном по адресу:</p>'
      + '<p class="center"><strong>' + escapeHtml(payload.address || payload.house || '') + '</strong></p>'
      + '<p class="indent">Будет проводиться внеочередное общее собрание собственников помещений в многоквартирном доме, расположенном по адресу: ' + escapeHtml(payload.address || payload.house || '') + '.</p>'
      + '<h2>Заочный этап голосования</h2>'
      + '<div class="meta"><strong>Дата начала ОСС:</strong><span>' + escapeHtml(start) + '</span><strong>Дата окончания приема решений:</strong><span>' + escapeHtml(end) + '</span><strong>Форма голосования:</strong><span>заочное голосование</span></div>'
      + '<p class="indent">Решение клиента необходимо заполнить и передать инициатору общего собрания по адресу, указанному управляющей организацией.</p>'
      + '<h2>Повестка дня общего собрания</h2>'
      + ossQuestionsHtml(questions)
      + '<p class="footer-note">После окончания заочного этапа будет произведен подсчет голосов и сформирован протокол.</p>'
      + '</main>';
    pdfDocumentShell('Сообщение ОСС PDF', body, { fileName: 'soobshchenie_OSS_' + safeFileName(payload.house || payload.address || 'dom') });
  }

  function ownerBallotPdfHtml(payload, owner, index, names) {
    const questions = payload.questions || [];
    const start = payload.startDate ? formatDate(new Date(payload.startDate)) : '';
    const end = payload.endDate ? formatDate(new Date(payload.endDate)) : '';
    return '<main class="sheet">'
      + '<div class="template-note">Шаблон бюллетеня: ' + escapeHtml(names.ballot) + ' · бюллетень ' + (index + 1) + '</div>'
      + '<h1>Решение клиента</h1>'
      + '<p class="center"><strong>помещения в многоквартирном доме, расположенном по адресу:</strong><br>' + escapeHtml(payload.address || payload.house || '') + '</p>'
      + '<p class="center">по вопросам внеочередного общего собрания собственников, проводимого в форме заочного голосования'
      + (start || end ? ' <span class="nowrap">с ' + escapeHtml(start) + ' по ' + escapeHtml(end) + '</span>' : '') + '.</p>'
      + '<table class="doc-table"><tbody>'
      + '<tr><th>№ помещения</th><th>Количество голосов, кв.м.</th></tr>'
      + '<tr><td class="center">' + escapeHtml(owner.flat) + '</td><td class="center">' + (owner.area ? formatNumber(owner.area) : '') + '</td></tr>'
      + '<tr><th>Клиент / наименование юридического лица</th><th>СНИЛС / ОГРН / ИНН</th></tr>'
      + '<tr><td>' + escapeHtml(owner.name) + '</td><td>' + escapeHtml(owner.snils || '') + '</td></tr>'
      + '<tr><th colspan="2">Сведения о праве собственности</th></tr>'
      + '<tr><td colspan="2">' + escapeHtml(owner.document || owner.share || '') + '</td></tr>'
      + '</tbody></table>'
      + '<h2>Вопросы, поставленные на голосование</h2>'
      + '<table class="doc-table"><thead><tr><th>№</th><th>Вопрос повестки</th><th class="vote">За</th><th class="vote">Против</th><th class="vote">Воздержался</th></tr></thead><tbody>'
      + questions.map((question, questionIndex) => '<tr><td class="center">' + (questionIndex + 1) + '</td><td>' + escapeHtml(question) + '</td><td></td><td></td><td></td></tr>').join('')
      + '</tbody></table>'
      + '<div class="sign-grid"><div><p>Дата заполнения</p><div class="sign-line"></div></div><div><p>Подпись клиента</p><div class="sign-line"></div></div></div>'
      + '</main>';
  }

  function openOssBallotsPdf() {
    const payload = selectedDocumentPayload();
    if (!payload) { showNotice('Выберите работу для бюллетеней.'); return; }
    const owners = validOwnersForDocuments();
    if (!owners.length) { showNotice('Загрузите корректный реестр клиентов для бюллетеней PDF.'); return; }
    const names = currentTemplateNames();
    const body = owners.map((owner, index) => ownerBallotPdfHtml(payload, owner, index, names)).join('');
    pdfDocumentShell('Бюллетени ОСС PDF', body, { fileName: 'bulletins_OSS_' + safeFileName(payload.house || payload.address || 'dom') + '_' + owners.length + '_clients' });
  }

  async function downloadExactOssNotice() {
    const payload = selectedDocumentPayload();
    if (!payload) { showNotice('Выберите работу для сообщения ОСС.'); return; }
    await postOssDocument({ ...payload, kind: 'notice', templateBase64: templateForKind('notice') });
  }

  async function downloadExactOssBallots() {
    const payload = selectedDocumentPayload();
    if (!payload) { showNotice('Выберите работу для бюллетеней.'); return; }
    const owners = validOwnersForDocuments();
    if (!owners.length) { showNotice('Загрузите корректный реестр клиентов для точных бюллетеней DOCX.'); return; }
    const templateBase64 = templateForKind('ballot');
    if (owners.length === 1) {
      showNotice('Формирую точный бюллетень DOCX...');
      await postOssDocument({ ...payload, kind: 'ballot', owners, templateBase64 });
      showNotice('Точный бюллетень DOCX сформирован.');
      return;
    }
    const batchSize = 25;
    const batches = [];
    for (let index = 0; index < owners.length; index += batchSize) {
      batches.push(owners.slice(index, index + batchSize));
    }
    for (let index = 0; index < batches.length; index += 1) {
      showNotice('Формирую точные бюллетени DOCX: партия ' + (index + 1) + ' из ' + batches.length + ' (' + batches[index].length + ' клиентов).');
      try {
        await postOssDocument({
          ...payload,
          kind: 'ballotsZip',
          owners: batches[index],
          templateBase64,
          fileSuffix: '_part_' + String(index + 1).padStart(2, '0') + '_of_' + String(batches.length).padStart(2, '0'),
        });
      } catch (error) {
        throw new Error('Не удалось сформировать партию ' + (index + 1) + ' из ' + batches.length + '. Если ошибка повторится, уменьшите реестр или попробуйте еще раз. Детали: ' + (error && error.message ? error.message : error));
      }
      await new Promise((resolve) => setTimeout(resolve, 300));
    }
    showNotice('Точные бюллетени DOCX сформированы партиями: ' + batches.length + ' ZIP-файлов.');
  }
  ...
  function wordDocument(title, body) {
    return '<!doctype html><html><head><meta charset="utf-8"><title>' + escapeHtml(title) + '</title><style>body{font-family:"Times New Roman",serif;font-size:12pt;line-height:1.35;color:#000}h1,h2{text-align:center;font-size:14pt}table{width:100%;border-collapse:collapse;margin:12px 0}td,th{border:1px solid #000;padding:6px;vertical-align:top}.line{display:inline-block;border-bottom:1px solid #000;min-width:260px}.right{text-align:right}.small{font-size:10pt}.no-border td{border:0}.page-break{page-break-after:always}</style></head><body>' + body + '</body></html>';
  }

  function downloadWordDoc(fileName, title, body) {
    downloadTextFile(fileName, wordDocument(title, body), 'application/msword;charset=utf-8');
  }

  function downloadRepairOssNotice(id) {
    const row = state.repairRows.find((item) => item.id === id);
    if (!row) return;
    const total = repairEstimateTotal(row);
    const quorum = repairOssQuorumTarget(row);
    const questions = repairOssQuestions(row);
    const body = '<h1>Сообщение о проведении общего собрания собственников</h1>'
      + '<p>Уважаемые клиенты многоквартирного дома по адресу: <strong>' + escapeHtml(row.address || row.house || '') + '</strong>.</p>'
      + '<p>Сообщаем о проведении общего собрания собственников помещений по вопросу выполнения работ по текущему ремонту.</p>'
      + '<p><strong>Вид работ:</strong> ' + escapeHtml(row.workType || '') + '</p>'
      + '<p><strong>Ориентировочная сметная стоимость:</strong> ' + (formatMoney(total) || 0) + ' руб.</p>'
      + '<p><strong>Требуемый кворум:</strong> ' + quorum + '%.</p>'
      + '<h2>Повестка собрания</h2><ol>' + questions.map((item) => '<li>' + escapeHtml(item) + '</li>').join('') + '</ol>'
      + '<p>Дата, время, место проведения и порядок приема решений заполняются менеджером клиентского сервиса перед запуском ОСС.</p>';
    downloadWordDoc('soobshchenie_OSS_' + safeFileName(row.house || row.address) + '.doc', 'Сообщение ОСС', body);
  }

  function downloadRepairOssBallot(id) {
    const row = state.repairRows.find((item) => item.id === id);
    if (!row) return;
    const total = repairEstimateTotal(row);
    const quorum = repairOssQuorumTarget(row);
    const questions = repairOssQuestions(row);
    const body = '<h1>Бюллетень для голосования на общем собрании собственников</h1>'
      + '<p><strong>Адрес МКД:</strong> ' + escapeHtml(row.address || row.house || '') + '</p>'
      + '<p><strong>Работы:</strong> ' + escapeHtml(row.workType || '') + '</p>'
      + '<p><strong>Сметная стоимость:</strong> ' + (formatMoney(total) || 0) + ' руб. <span class="small">Требуемый кворум: ' + quorum + '%.</span></p>'
      + '<table class="no-border"><tr><td>Ф.И.О. собственника: <span class="line"></span></td><td>Квартира/помещение № <span class="line"></span></td></tr><tr><td>Документ собственности: <span class="line"></span></td><td>Площадь помещения: <span class="line"></span></td></tr></table>'
      + '<table><thead><tr><th>№</th><th>Вопрос повестки</th><th>За</th><th>Против</th><th>Воздержался</th></tr></thead><tbody>' + questions.map((item, index) => '<tr><td>' + (index + 1) + '</td><td>' + escapeHtml(item) + '</td><td></td><td></td><td></td></tr>').join('') + '</tbody></table>'
      + '<p>Дата заполнения: <span class="line"></span> Подпись: <span class="line"></span></p>'
      + '<p class="small">Бюллетень сформирован по данным дашборда. Перед печатью менеджер клиентского сервиса сверяет реквизиты собрания и формулировки вопросов.</p>';
