(function () {
  ...
  function renderGantt() {
    ...
    grid.style.setProperty('--gantt-columns', columns.map((column) => column.kind === 'month' ? '84px' : '22px').join(' '));
    ...
    columns.forEach((column) => {
      ...
      head.className = 'gantt-head' + (column.kind === 'month' ? ' gantt-month-head' : ' gantt-day-head');
      head.textContent = column.label;
      if (column.kind === 'month') {
        if (state.expandedGanttMonths && state.expandedGanttMonths.has(column.key)) head.classList.add('expanded');
        head.title = 'Раскрыть месяц';
        head.dataset.monthKey = column.key;
        head.addEventListener('click', () => toggleGanttMonth(column.key));
      }
    ...
    while (cursor <= to) {
      ...
      if (state.expandedGanttMonths && state.expandedGanttMonths.has(key)) {
        for (let day = new Date(start); day <= end; day = addDays(day, 1)) {
          columns.push({ kind: 'day', key: monthKey(day) + '-' + day.getDate(), label: day.getDate() === 1 ? formatShortMonth(day) : String(day.getDate()), start: day, end: day });
        }
      } else {
